...
public class AurigaProtocollaPostaElettronicaDataSource extends AbstractServiceDataSource<DettaglioPostaElettronicaBean, ProtocollazioneBean> {
	...
	public ProtocollazioneBean call(DettaglioPostaElettronicaBean bean) throws Exception {
		...
		Map<String, Object> lrecuperaFilesMap = recuperaFiles(lTEmailMgoBean, tipoConversione);

		Map<String, Object> lFiles = (Map<String, Object>) lrecuperaFilesMap.get("file");
		...
		DocumentoXmlOutBean lDocumentoXmlOutBean = preparaRegistrazione(bean, lFileSegnaturaXml, lFiles);
		...
		if (bean.getFileDaAppunti() != null && !bean.getFileDaAppunti().isEmpty()) {
		...
		if (isAttivaDecompressionePerMailEntrata(lDocumentoXmlOutBean.getFlgTipoProv())) {
...
//			for(AllegatiOutBean item : lDocumentoXmlOutBean.getAllegati()) {
//				System.out.println("Finale: "+item.getNomeOriginale()+"\n");
//			}
		}
		...
		if(StringUtils.isNotBlank(lDocumentoXmlOutBean.getIdUDFromEmail())) {
			...
			if (lrecuperaFilesMap.get("isConvSucces") != null && (Boolean) lrecuperaFilesMap.get("isConvSucces")) {
		...
		if (lEmailAttachsBean.getFiles() != null && !lEmailAttachsBean.getFiles().isEmpty()) {
			...
			for (File lFile : lEmailAttachsBean.getFiles()) {
				...
				if (filename != null) {
					...
					listEmlAllegatiBean.add(createAllegato(algoritmoImpronta, encodingImpronta, new BigDecimal(size), filename, mimeType, impronta));
	...
	protected DocumentoXmlOutBean preparaRegistrazione(DettaglioPostaElettronicaBean pInBean, String lFileSegnaturaXml, Map<String, Object> lFiles)
			throws Exception {
	...
	private void sistemaArchivioEstratto(DocumentoXmlOutBean lDocumentoXmlOutBean, Map<String, Object> lFiles, Map<String, String> listFileUriExtract,
			...
			try {
				...
				if (listFileUriExtract.size() == 1 && isFilePrimario) {
					...
					AllegatiOutBean bean = createAllegato(algoritmoImpronta, encodingImpronta, BigDecimal.valueOf(fileExt.length()), fileName, mimeType,
							impronta);
					//System.out.println("Aggiungo file: "+ bean.getDisplayFileName());
		...
		if (isFilePrimario && listFileUriExtract.size() != 1) {
			...
			for (AllegatiOutBean allegato : listaAllegatiNew) {
				if (allegato.getNomeOriginale().contains(nomeArchivio)) {
					//System.out.println("Rimuovo file: "+nomeArchivio);
		...
		for (Entry<String, Object> lFile : lFiles.entrySet()) {
			if (lFile.getKey().contains(nomeArchivio)) {
	...
	private AllegatiOutBean createAllegato(String algoritmoImpronta, String encodingImpronta, BigDecimal dimension, String fileName, String mimeType,
		...
		try {
			bean.setFlgConvertibilePdf(FormatiUtil.isConvertibile(getSession(), mimeType) ? Flag.SETTED : Flag.NOT_SETTED);
		} catch (Exception e) {
			bean.setFlgConvertibilePdf(Flag.NOT_SETTED);
			mLogger.error("Errore nel settaggio del flag Convertibile per il file: " + fileName, e);
		}
		return bean;
	}
	...
	private FilePrimarioOutBean createFilePrimario(String algoritmoImpronta, String encodingImpronta, BigDecimal dimension, String fileName, String mimeType,
		...
		try {
			filePrimarioNew.setFlgConvertibilePdf(FormatiUtil.isConvertibile(getSession(), mimeType) ? Flag.SETTED : Flag.NOT_SETTED);
		} catch (Exception e) {
			filePrimarioNew.setFlgConvertibilePdf(Flag.NOT_SETTED);
			mLogger.error("Errore nel settaggio del flag Convertibile per il file: " + fileName, e);
		}
	...
	// /**
	// *
	// * @param filesListExtr lista di file estratti
	// * @return un valore booleano che indica se tra i file di input ne esiste almeno uno protetto da password.
	// */
	// private boolean checkProtezioneFile(List<File> filesListExtr){
	// for(File lFile : filesListExtr){
	// String esito = FileProtetto.checkPasswordProtection(lFile);
	// if(esito != null && esito.contains("Errore")){
	// //il file è protetto
	// return true;
	// }
	// }
		...
		for (ItemLavorazioneMailBean itemLavorazioneBean : fileDaAppunti) {
			...
			String algoritmoImpronta = StringUtils.isNotBlank(itemLavorazioneBean.getItemLavAlgoritmoImprontaFile())
					...
					String encodingImpronta = StringUtils.isNotBlank(itemLavorazioneBean.getItemLavAlgoritmoEncodingImprontaFile())
							...
							AllegatiOutBean allegatoBean = createAllegato(algoritmoImpronta, encodingImpronta, new BigDecimal(size), filename, mimeType, impronta);

...
public class AurigaGestioneUtentiDataSource extends AurigaAbstractFetchDatasource<GestioneUtentiBean> {
	...
	public GestioneUtentiBean getLoaddettuserestesa(GestioneUtentiBean bean) throws Exception {
		...
		result.setIdMansione(output.getResultBean().getQualificaout());
		result.setNomeMansione(output.getResultBean().getQualificaout());
		result.setCodiceUtenteMansione(output.getResultBean().getCodrapidoout());

		result.setEmail(output.getResultBean().getEmailout());
		result.setDtIniVld(output.getResultBean().getDtiniziovldout() != null ? new SimpleDateFormat(FMT_STD_DATA).parse(output.getResultBean()
		...
		if (listaAttributi != null && listaAttributi.size() > 0) {
			...
			List<AreeCommercialiBean> listaAreeCommerciali = new ArrayList<AreeCommercialiBean>();
			...
			for (AttributoBean lAttributoBean : listaAttributi) {
				...
				for (HashMap<String, String> valori : valoriLista) {
					for (String key : valori.keySet()) {
						...
						// Se e' un AREA COMMERCIALE DI COMPETENZA
						else if (key.equalsIgnoreCase("COD_AREA_COMM_COMPETENZA")) {
							if (valoreAttributo != null && !valoreAttributo.equalsIgnoreCase("")) {
								AreeCommercialiBean lAreeCommercialiBean = new AreeCommercialiBean();
								lAreeCommercialiBean.setIdAreaCommerciali(valori.get(key));
								listaAreeCommerciali.add(lAreeCommercialiBean);
							}
						}
			...
			result.setListaAreeCommerciali(listaAreeCommerciali);
		}
	...
	public GestioneUtentiBean add(GestioneUtentiBean bean) throws Exception {
		...
		boolean isGestioneAreaCommerciale            = Boolean.parseBoolean(getExtraparams().get("isGestioneAreaCommerciale"));
		...
		input.setCodrapidoin(bean.getCodiceUtenteMansione());
		...
		popolaAttributiDinamici(isGestioneLingua, isGestioneLogo, isGestioneUtenteInterno,  isGestioneClienti, isGestioneSocieta,  isGestioneAreaCommerciale, isVisibilitaEmailCaselleAssociateUO, bean, valoriAttributiDinamici, tipiValoriAttributiDinamici);
	...
	protected String getXmlUoAssociateUtente(GestioneUtentiBean bean, String callFrom) throws JAXBException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		...
		for (UoAssociateUtenteBean lUoAssociateUtenteBean : bean.getListaUoAssociateUtente()) {
			...
			lUoAssociateUtenteXmlBean.setFlgRuoloPrimario("1");
			lUoAssociateUtenteXmlBean.setDenominazioneScrivaniaUtente(lUoAssociateUtenteBean.getDenominazioneScrivaniaUtente());
			lUoAssociateUtenteXmlBean.setFlgIncluseSottoUo(lUoAssociateUtenteBean.isFlgIncluseSottoUo() ? "1" : "0");
			lUoAssociateUtenteXmlBean.setFlgIncluseScrivanie(lUoAssociateUtenteBean.isFlgIncluseScrivanie() ? "1" : "0");
			...
			// FIXME Devo aggiungere le informazioni dell'eretidarietà delle abilitazioni
    ...
    protected void popolaAttributiDinamici(boolean isGestioneLingua, boolean isGestioneLogo, boolean isGestioneUtenteInterno , boolean isGestioneClienti, boolean isGestioneSocieta, boolean isGestioneAreaCommerciale, boolean isVisibilitaEmailCaselleAssociateUO, GestioneUtentiBean bean , Map<String, Object> valoriAttributiDinamici,  Map<String, String> tipiValoriAttributiDinamici) throws Exception {
		...
		// Lista con le aree commerciali
		if(isGestioneAreaCommerciale){
			List<HashMap<String, String>> valoriLista = new ArrayList<HashMap<String, String>>();
			if (bean.getListaAreeCommerciali() != null && bean.getListaAreeCommerciali().size() > 0) {
				for (AreeCommercialiBean lAreeCommercialiBean : bean.getListaAreeCommerciali()) {
					HashMap<String, String> lMap = new HashMap<String, String>();
					lMap.put("1", lAreeCommercialiBean.getIdAreaCommerciali());
					valoriLista.add(lMap);
				}
			}
			valoriAttributiDinamici.put("COD_AREA_COMM_COMPETENZA", valoriLista);
			tipiValoriAttributiDinamici.put("COD_AREA_COMM_COMPETENZA", (String)"LISTA");
    	}
	...
	protected DmpkDefSecurityTrovausersBean createFetchInput(AurigaLoginBean loginBean, AdvancedCriteria criteria, String token, String idUserLavoro,Integer overflowLimit) throws Exception, JAXBException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		...
		if (criteria != null && criteria.getCriteria() != null) {
			for (Criterion crit : criteria.getCriteria()) {
				...
				} else if ("mansioneUtente".equals(crit.getFieldName())) {
					mansioneUtente = getTextFilterValue(crit);
		...
		input.setQualificaio(mansioneUtente);		

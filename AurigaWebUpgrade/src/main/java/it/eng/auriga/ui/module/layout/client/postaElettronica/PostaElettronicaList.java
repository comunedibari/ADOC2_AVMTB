...
public class PostaElettronicaList extends CustomList {
			...
			if (abilitaProtocollaAccessoAttiSUE) {
				...
				protocollaAccessoAttiSUEMenuItem.addClickHandler(new com.smartgwt.client.widgets.menu.events.ClickHandler() {
					...
					public void onClick(MenuItemClickEvent event) {
						...
						protocollaAccessoAttiSUEClick(pRecordMail, destinatariEAccount);
					}
				});
	...
	protected void manageProtocollaClick(final Record recordList, final String destinatariEAccount, final String idTipoDoc, final String nomeTipoDoc, final boolean richiestaAcessoAttiSue) {
		...
		lGwtRestDataSource.performCustomOperation("get", recordList, new DSCallback() {
...
//				final String tipoDestinatarioPreferito = currentRecord.getAttribute("tipoDestinatarioPreferito");
		...
		if(!AurigaLayout.getImpostazioniDocumentoAsBoolean("skipSceltaTipologiaEntrata")) {
			...
			AurigaLayout.apriSceltaTipoDocPopup(false, idTipoDocDefault, "PG", null, new ServiceCallback<Record>() {
				...
				public void execute(Record lRecordTipoDoc) {
					...
					manageProtocollaClick(pRecordMail, destinatariEAccount, idTipoDoc, nomeTipoDoc, false);
				}
			});
		} else {
	...
	private void protocollaAccessoAttiSUEClick(final Record pRecordMail, final String destinatariEAccount) {
		
		Record mittente = new Record();
		mittente.setAttribute("emailMittente", pRecordMail.getAttribute("accountMittente"));
		mittente.setAttribute("tipoMittente", "PF");
		final RecordList listaMittenti = new RecordList();
		listaMittenti.add(mittente);
		pRecordMail.setAttribute("listaEsibenti", listaMittenti);
		...
		AurigaLayout.apriSceltaTipoRichiestaAccessoAttiPopup(null, new ServiceCallback<Record>() {
			...
			public void execute(Record lRecordTipoDoc) {
				...
				manageProtocollaClick(pRecordMail, destinatariEAccount, idTipoDoc, nomeTipoDoc, true);
			}
		});
	}
	
	private void protocollaAccessoAttiSUEClick_OLD(final Record recordMail, final String destinatariEAccount) {
...
//		indirizzoRiferimento.setAttribute("civico", "1");
		...
		Record mittente = new Record();
		mittente.setAttribute("emailMittente", recordMail.getAttribute("accountMittente"));
		mittente.setAttribute("tipoMittente", "PF");
		final RecordList listaMittenti = new RecordList();
		listaMittenti.add(mittente);
		
		final String idEmail = recordMail.getAttribute("idEmail");

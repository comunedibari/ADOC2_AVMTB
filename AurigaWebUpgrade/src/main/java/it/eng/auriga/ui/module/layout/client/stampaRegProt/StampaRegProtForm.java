...
public class StampaRegProtForm extends DynamicForm{
	...
	public StampaRegProtForm(String nomeEntita){
...
//		else{
		...
		SelectGWTRestDataSource lGwtRestDataSource = new SelectGWTRestDataSource("LoadComboStampaRegistro", new String[]{"codice","descrizione"}, true);
			...
			public void execute(DSResponse response, Object rawData, DSRequest request) {
		        ...
		        if(data != null && data.getLength() > 0) {
					for (int i = 0; i < data.getLength(); i++) {			        	
			        	if(data.get(i).getAttribute("idRegistro") != null && !data.get(i).getAttribute("idRegistro").equals("")) {			        	
			        		valueMap.put(data.get(i).getAttribute("idRegistro"), data.get(i).getAttribute("voceRegistro").equals(null) ? "" : data.get(i).getAttribute("voceRegistro"));
			        	}			        	
					}
...
//		}
		...
		tipoIntervalloMap.put("D", "di date di registrazione");  
		...
		nroRegDaItem = new NumericItem("nroRegDa", "N.ro di registrazione dal");
		...
		dataRegDaItem = new DateItem("dataRegDa", "Data di registrazione dal");

	...
	protected void buildIndirizzoForm() {
		...
		if (!isClientToponomasticaAttivo()) {
			...
			if (isIndirizzoObbligatorio()) {
				...
				indirizzoItem.setValidators(indirizzoInComuneValidator);
			}
			...
			((ComboBoxItem) indirizzoItem).setValidators(lCustomValidator);
			...
			indirizzoItem.setItemHoverFormatter(new FormItemHoverFormatter() {

				@Override
				public String getHoverHTML(FormItem item, DynamicForm form) {

					return (String) indirizzoItem.getValue();
				}
			});
...
//			}
		...
		if (isIndirizzoObbligatorio()) {
			capItem.setAttribute("obbligatorio", !isForceCapNonObbligatorio());
			capItem.setValidators(new RequiredIfValidator(new RequiredIfFunction() {

				@Override
				public boolean execute(FormItem formItem, Object value) {
					return isStatoItalia() && !isForceCapNonObbligatorio();
				}
			}));
		}
...
//	}
	...
	protected void setIndirizzoCompletoFromRecordRubrica(Record lRecord) {
		...
		if (ProtocollazioneUtil.isAttivoProtocolloWizard(null)) {
			if(lRecord != null) {
				if (lRecord.getAttribute("indirizzoCompleto") != null && !"".equals(lRecord.getAttribute("indirizzoCompleto"))) {
					...
					try {
						...
						record.setAttribute("comune", comuneCitta);
						record.setAttribute("nomeComune", codIstat);
	...
	protected void controllaIndirizzoSuClientToponomastica() {
		String codiceStada = mIndirizzoDynamicForm1.getValueAsString("codToponimo");
		...
		if (codiceStada != null && !"".equalsIgnoreCase(codiceStada) && numeroCivico != null && !"".equalsIgnoreCase(numeroCivico)) {
			Record lRecord = new Record();
			lRecord.setAttribute("codiceStrada", codiceStada);

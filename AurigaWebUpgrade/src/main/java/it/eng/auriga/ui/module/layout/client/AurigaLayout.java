...
import it.eng.auriga.ui.module.layout.client.pratiche.ApriDettaglioPraticaSenzaFlussoLayout;
...
public class AurigaLayout extends Layout {
	...
	protected static LinkedHashMap<String, Record> destProtEntrataDefaultMap;
			...
			protected void viewMessages(MessageBean message, final Integer waitingTime, final WaitingMessageCallback callback) {
				if (AurigaLayout.getIsAttivaAccessibilita() || AurigaLayout.getParametroDBAsBoolean("SHOW_POPUP_GUI_MESSAGES")) {
					if (message.getType().equals(MessageType.INFO) || message.getType().equals(MessageType.WARNING_NO_POPUP)) {
						super.viewMessages(message, waitingTime, callback);
					} else {
	...
	public void updateWelcomeMessage() {
		if (utenteLoggato != null) {
			...
			if (uoLavoro != null && !"".equals(uoLavoro)) {
				String desUoLavoro = uoCollegateUtenteValueMap != null ? uoCollegateUtenteValueMap.get(uoLavoro) : null;
...
//		});
	...
	public void caricaDatiUtenteLavoro(final DSCallback callback) {
		new GWTRestDataSource("UoCollegateUtenteDatasource", "idUo", FieldType.TEXT).fetchData(null, new DSCallback() {

			@Override
			public void execute(DSResponse response, Object rawData, DSRequest request) {

				Record[] data = response.getData();
				uoCollegateUtenteValueMap = new LinkedHashMap<String, String>();
				destProtEntrataDefaultMap = new LinkedHashMap<String, Record>();
				if (data.length > 0) {
					for (int i = 0; i < data.length; i++) {
						uoCollegateUtenteValueMap.put(data[i].getAttribute("idUo"), data[i].getAttribute("descrizione"));
						boolean flgPreimpDestProtEntrata = data[i].getAttribute("flgPreimpDestProtEntrata") != null && "1".equals(data[i].getAttribute("flgPreimpDestProtEntrata"));
						if(flgPreimpDestProtEntrata) {
							destProtEntrataDefaultMap.put(data[i].getAttribute("idUo"), data[i]);			
						}						
					}
				}
	...
	public void afterAggiornaUtente() {
		 ...
		 */
		loadUserPreference("docScrivania", "DEFAULT", utenteLoggato.getIdUtente(), new ServiceCallback<Record>() {
		...
		if (getParametroDB("NEWS_TO_SHOW") != null && !"".equals(getParametroDB("NEWS_TO_SHOW"))) {
			showNewsAlert();
		}
...
//			}
	...
	public static void apriDettaglioPraticaSenzaFlusso(Record lRecord, String title, boolean isEditMode) {
		new ApriDettaglioPraticaSenzaFlussoLayout(lRecord, title, isEditMode);
	}
			...
			idTipoDocumentoDS.fetchData(null, new DSCallback() {
				...
				public void execute(DSResponse response, Object rawData, DSRequest request) {
					...
					if (response.getData().length == 0 || ((required || isRepertorio) && response.getData().length == 1)) {
						...
						new SceltaTipoDocPopup(required, idTipoDocDefault,null, categoriaReg, siglaReg, callback);
					}
				}
			});
	...
	public static void apriSceltaCommissione(final ServiceCallback<Record> callback) {
		...
		convocazioneCommissioneDS.fetchData(null, new DSCallback() {
			...
			public void execute(DSResponse response, Object rawData, DSRequest request) {
				...
				if (response.getData().length == 0) {
					Layout.addMessage(new MessageBean("Nessun atto da discutere in commissione", "", MessageType.ERROR));
				} else if (response.getData().length == 1) {
					Record lRecord = new Record();
	...
	public List<MenuItem> buildConfigUtenteMenuItems(final Menu menuBarraDesktop) {
		...
		impostazioniFirmaMenuItem.addClickHandler(new com.smartgwt.client.widgets.menu.events.ClickHandler() {
...
//		recInput.setAttribute("impostazioniAperturaDettaglioPerRicercaPuntuale", impostazioniAperturaDettaglioPerRicercaPuntuale);
			...
			public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
				if (dsResponse.getData() != null) {
					...
					if (impostazioniFirma != null && impostazioniFirma.getAttributes() != null && impostazioniFirma.getAttributes().length > 0) {
						saveImpostazioniFirmaWindow.setValues(impostazioniFirma);
					} else {
						...
						saveImpostazioniFirmaWindow.setValues(recordImpostazioni);
					}
	...
	public static LinkedHashMap<String, Record> getDestProtEntrataDefaultMap() {
		...
		ret.putAll(destProtEntrataDefaultMap);
		return ret;
	}
	
	// questo metodo ritorna la mappa di tutte le UO associate all'utente se non è settata una UO di lavoro predefinita o su cui è attiva la funzione di protocollo, altrimenti ritorna solo quella
	// se ho un'unica UO collegata all'utente il metodo ovviamente mi restituirà solo quella 
	public static LinkedHashMap<String, String> getUoProtocollantiValueMap() {
		LinkedHashMap<String, String> ret = new LinkedHashMap<String, String>();
		String uoPuntoProtAttivato = idUOPuntoProtAttivato != null && !"".equals(idUOPuntoProtAttivato) ? "UO" + idUOPuntoProtAttivato : null;
		if( uoPuntoProtAttivato != null && !"".equals(uoPuntoProtAttivato) && uoCollegateUtenteValueMap.get(uoPuntoProtAttivato) != null){
			ret.put(uoPuntoProtAttivato, uoCollegateUtenteValueMap.get(uoPuntoProtAttivato));
		} else if (uoLavoro != null && !"".equals(uoLavoro) && uoCollegateUtenteValueMap.get(uoLavoro) != null) {
	...
	public void showNewsAlert() {
		loadUserPreference("skipNewsAlert", "DEFAULT", new ServiceCallback<Record>() {
			...
			public void execute(Record recordPref) {
				...
				if (!skipNewsAlert) {
					NewsDialogBox newsDialogBox = new NewsDialogBox("News", getParametroDB("NEWS_TO_SHOW")) {

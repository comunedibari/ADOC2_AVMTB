...
public class AvvioIterAttiActivitiDatasource extends AbstractServiceDataSource<AvvioIterAttiInBean, AvvioIterAttiOutBean>{
	...
	private void addPropostaDiscussione(AvvioIterAttiInBean pInBean) throws Exception {
		...
		if (resultDSBean.getListaArgomentiOdg() != null && !resultDSBean.getListaArgomentiOdg().isEmpty()) {
			for (int i = 0; i < resultDSBean.getListaArgomentiOdg().size(); i++) {
				ArgomentiOdgXmlBean item = resultDSBean.getListaArgomentiOdg().get(i);
				int nrOrdineOdg = item.getNrOrdineOdg() != null ? item.getNrOrdineOdg() : 0;
				if (nrOrdineOdg > nrOrdineOdgMax) {
					nrOrdineOdgMax = nrOrdineOdg;
				}
			}
		}
		lArgomentiOdgXmlBean.setNrOrdineOdg(nrOrdineOdgMax+1);
	...
	private void addPropostaConvocazione(AvvioIterAttiInBean pInBean) throws Exception {
		...
		lArgomentiOdgXmlBean.setOggetto(pInBean.getOggetto());
		int nrOrdineOdgMax = 0;
		if (resultCSBean.getListaArgomentiOdg() != null && !resultCSBean.getListaArgomentiOdg().isEmpty()) {
			for (int i = 0; i < resultCSBean.getListaArgomentiOdg().size(); i++) {
				ArgomentiOdgXmlBean item = resultCSBean.getListaArgomentiOdg().get(i);
				int nrOrdineOdg = item.getNrOrdineOdg() != null ? item.getNrOrdineOdg() : 0;
				if (nrOrdineOdg > nrOrdineOdgMax) {
					nrOrdineOdgMax = nrOrdineOdg;
				}

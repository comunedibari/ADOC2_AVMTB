...
public class GestioneUtentiList extends CustomList {
	...
	private ListGridField codiceUtenteMansione;
	...
	public GestioneUtentiList(String nomeEntita) {
		...
		codiceUtenteMansione  = new ListGridField("codiceUtenteMansione", I18NUtil.getMessages().gestioneutenti_codiceUtenteMansione());
		...
		String nomeLabelQualifica = I18NUtil.getMessages().gestioneutenti_qualifica();
		// Cambio la label 
		if (getApplicationName() != null ){
			
			// Se l'applicazione e' EIIFACT
			if (getApplicationName().equalsIgnoreCase("EIIFact")) {
				
			   // Se il parametro ATTIVA_GEST_MANSIONE = true la label e' quella della MANSIONE
				if(isGestioneMansione()) {
					nomeLabelQualifica = I18NUtil.getMessages().gestioneutenti_mansione();
				}
				// Se il parametro ATTIVA_GEST_MANSIONE = false la label e' quella della QUALIFICA
				else{
					nomeLabelQualifica = I18NUtil.getMessages().gestioneutenti_qualifica();
				}
			}		
		}		
		qualifica             = new ListGridField("qualifica", nomeLabelQualifica);
		...
		// Nascondo i campi campi se l'applicazione e' EIIFACT
		if (getApplicationName() != null && (getApplicationName().equalsIgnoreCase("EIIFact"))) {
			
			titolo.setHidden(true);
			titolo.setCanHide(false);
			nroMatricola.setHidden(true);
			nroMatricola.setCanHide(false);
			
			// Se il parametro ATTIVA_GEST_MANSIONE = true
			if(isGestioneMansione()) {
				// mostro la colonna QUALIFICA 
				qualifica.setHidden(false);
				qualifica.setCanHide(true);
				
				// mostro la colonna CODICE UTENTE MANSIONE 
				codiceUtenteMansione.setHidden(false);
				codiceUtenteMansione.setCanHide(true);
			}
			// Se il parametro ATTIVA_GEST_MANSIONE = false
			else {
				// nascondo la colonna QUALIFICA 
				qualifica.setHidden(true);
				qualifica.setCanHide(false);
				
				// nascondo la colonna CODICE UTENTE MANSIONE 
				codiceUtenteMansione.setHidden(true);
				codiceUtenteMansione.setCanHide(false);				
			}
		}
		...
		setFields(idUser, cognomeNome, username, indirizzo, nomeProfilo, nroMatricola, qualifica, codiceUtenteMansione, titolo, email, dtIniVld, dtFineVld, idSoggRubrica, flgUtenteBloccatoMsgHover, flgAccreditatiInDomIo, attivoIn, dtUltimoAccesso, codiceUO, nomeAppUO, flgUtenteBloccatoIcon);
	...
	public void setImpostazioniGenerali() {
		...
		setEfactPersonalizzazioni(AurigaLayout.getParametroDBAsBoolean("EFACT_PERSONALIZZAZIONI"));
		setGestioneMansione(AurigaLayout.getParametroDBAsBoolean("ATTIVA_GEST_MANSIONE"));		
		// nota. Ho disabilitato la sezione dei commerciali.
		setGestioneAreaCommerciale(false);
	}
	...
	public boolean isEfactPersonalizzazioni() {
		return eFactPersonalizzazioni;
	}
	public void setEfactPersonalizzazioni(boolean eFactPersonalizzazioni) {
		this.eFactPersonalizzazioni = eFactPersonalizzazioni;
	}	
	...
	public boolean isGestioneMansione() {
		return gestioneMansione;
	}

	public void setGestioneMansione(boolean gestioneMansione) {
		this.gestioneMansione = gestioneMansione;
	}
	...
	public boolean isGestioneAreaCommerciale() {
		return gestioneAreaCommerciale;
	}

	public void setGestioneAreaCommerciale(boolean gestioneAreaCommerciale) {
		this.gestioneAreaCommerciale = gestioneAreaCommerciale;
	}

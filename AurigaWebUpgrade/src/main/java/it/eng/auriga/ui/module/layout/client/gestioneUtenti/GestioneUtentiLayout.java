...
public class GestioneUtentiLayout extends CustomLayout{
	...
	private static GestioneUtentiFilter createFilter(String nomeEntita){
		...
		boolean showFilterMansione = AurigaLayout.getParametroDBAsBoolean("ATTIVA_GEST_MANSIONE");
		String productInstName = AurigaLayout.getParametroDB("PRODUCT_INST_NAME");
		if(productInstName==null)
			productInstName = "";
		if(productInstName.equalsIgnoreCase("DiGiDoc")){
			showFilterEscludiUtentiNoDominio = false;
		}
		extraParam.put("showFilterEscludiUtentiNoDominio", Boolean.toString(showFilterEscludiUtentiNoDominio));
		extraParam.put("showFilterMansione", Boolean.toString(showFilterMansione));
	...
	public void onSaveButtonClick() {
		...
		((GWTRestDataSource)(getDetail().getDataSource())).extraparam.put("isEfactPersonalizzazioni", Boolean.toString(isEfactPersonalizzazioni()));		
		((GWTRestDataSource)(getDetail().getDataSource())).extraparam.put("isGestioneAreaCommerciale", Boolean.toString(isGestioneAreaCommerciale()));	

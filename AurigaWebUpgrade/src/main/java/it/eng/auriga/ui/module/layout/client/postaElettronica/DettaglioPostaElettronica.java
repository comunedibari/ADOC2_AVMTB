...
//	protected DestinatariPrincipaliItem destinatariCCNAttoriItem;// ReplicableItem
	protected DestinatariPrincipaliGridItem destinatariPrincipaliAttoriItem;// GridItem
	protected DestinatariPrincipaliGridItem destinatariCopiaAttoriItem;// GridItem
	protected DestinatariPrincipaliGridItem destinatariCCNAttoriItem;// GridItem
	...
	private ModalWindow richAccessoAttiSUEmodalWindow;
...
//			}
//		});
		destinatariPrincipaliAttoriItem = new DestinatariPrincipaliGridItem("listaDestinatariPrincipali", I18NUtil.getMessages().posta_elettronica_destinatari_principali_attori_item());
		destinatariPrincipaliAttoriItem.setColSpan(7);
...
//			}
//		});
		destinatariCopiaAttoriItem = new DestinatariPrincipaliGridItem("listaDestinatariCC", I18NUtil.getMessages().posta_destinatari_copia_attori_item());
		destinatariCopiaAttoriItem.setColSpan(7);
...
//			}
//		});
		destinatariCCNAttoriItem = new DestinatariPrincipaliGridItem("listaDestinatariCCN", I18NUtil.getMessages().posta_destinatari_copia_ccn_attori_item());
		destinatariCCNAttoriItem.setColSpan(7);
	...
	protected void manageRegistraIstanzaClick(final Record pRecord, final String nomeEntita) {
		...
		lGwtRestServiceRegistraIstanza.call(pRecord, new ServiceCallback<Record>() {
			...
			public void execute(final Record object) {
				if (object.getAttribute("idUd") != null && !object.getAttribute("idUd").trim().equals("")) {
...
//		firmaPredefinitaButton.setBackgroundImage("");
	...
	private void manageRegistraProtocollaCick() {
		...
		final Record recordProtocolla = new Record();
		recordProtocolla.setAttribute("idEmail", new Record(vm.getValues()).getAttribute("idEmail"));
		recordProtocolla.setAttribute("flgIo", new Record(vm.getValues()).getAttribute("flgIO"));
		recordProtocolla.setAttribute("tsInvio", new Record(vm.getValues()).getAttribute("tsInvio"));
		recordProtocolla.setAttribute("body", new Record(vm.getValues()).getAttribute("body"));
		recordProtocolla.setAttribute("categoria", new Record(vm.getValues()).getAttribute("categoria"));
		...
		if(!AurigaLayout.getImpostazioniDocumentoAsBoolean("skipSceltaTipologiaEntrata")) {
			...
			AurigaLayout.apriSceltaTipoDocPopup(false, idTipoDocDefault, "PG", null, new ServiceCallback<Record>() {
				...
				public void execute(Record lRecordTipoDoc) {
					...
					manageProtocollaClick(recordProtocolla, destinatariEAccount, idTipoDoc, nomeTipoDoc, false);
				}
			});
		} else {
	...
	protected void manageProtocollaClick(final Record pRecord, final String destinatariEAccount, final String idTipoDoc, final String nomeTipoDoc, final boolean richiestaAcessoAttiSue) {
						...
						lGwtRestServiceProtocolla.call(pRecord, new ServiceCallback<Record>() {
							...
							public void execute(final Record object) {
								if (object.getAttribute("idUd") != null && !object.getAttribute("idUd").trim().equals("")) {
									...
									manageProtocollaPostaElettronicaCallback(object, destinatariEAccount, false, null, richiestaAcessoAttiSue);
								}
							}
						});
					} else {
			...
			public void execute(DSResponse response, Object rawData, DSRequest request) {
				if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
					final Record detailRecord = response.getData()[0];
						manageProtocollaPostaElettronicaCallback(detailRecord, destinatariEAccount, true, recordMail, false);
				}
			}
	...
	protected void manageProtocollaPostaElettronicaCallback(Record lRecord, String destinatariEAccount, boolean isThereIdUdMail, Record recordMail, boolean richiestaAcessoAttiSue) {
	...
	private void protocollaAccessoAttiSUEClick() {
		
		final Record recordProtocolla = new Record(vm.getValues());
//		recordProtocolla.setAttribute("idEmail", new Record(vm.getValues()).getAttribute("idEmail"));
//		recordProtocolla.setAttribute("flgIo", new Record(vm.getValues()).getAttribute("flgIO"));
//		recordProtocolla.setAttribute("tsInvio", new Record(vm.getValues()).getAttribute("tsInvio"));
//		recordProtocolla.setAttribute("body", new Record(vm.getValues()).getAttribute("body"));
//		recordProtocolla.setAttribute("categoria", new Record(vm.getValues()).getAttribute("categoria"));
		...
		mittente.setAttribute("emailMittente", recordProtocolla.getAttribute("accountMittente"));
		mittente.setAttribute("tipoMittente", "PF");
		final RecordList listaMittenti = new RecordList();
		listaMittenti.add(mittente);
		recordProtocolla.setAttribute("listaEsibenti", listaMittenti);
		...
		AurigaLayout.apriSceltaTipoRichiestaAccessoAttiPopup(null, new ServiceCallback<Record>() {
			...
			public void execute(Record lRecordTipoDoc) {
				...
				manageProtocollaClick(recordProtocolla, destinatariEAccount, idTipoDoc, nomeTipoDoc, true);
			}
		});
	}
	
	private void protocollaAccessoAttiSUEClick_OLD() {
		Record detailRecord = new Record(vm.getValues());
		
		Record indirizzoRiferimento = new Record();
//		indirizzoRiferimento.setAttribute("indirizzo", "VIA PALLETTA GIOVANNI BATTISTA");
//		indirizzoRiferimento.setAttribute("civico", "1");
		indirizzoRiferimento.setAttribute("indirizzo", "");
		indirizzoRiferimento.setAttribute("civico", "");
		final RecordList listaIndirizziRiferimento = new RecordList();
		listaIndirizziRiferimento.add(indirizzoRiferimento);
		
		Record mittente = new Record();
		mittente.setAttribute("emailMittente", detailRecord.getAttribute("accountMittente"));
		mittente.setAttribute("tipoMittente", "PF");
		final RecordList listaMittenti = new RecordList();
		listaMittenti.add(mittente);
		...
		AurigaLayout.apriSceltaTipoRichiestaAccessoAttiPopup(null, new ServiceCallback<Record>() {
			...
			public void execute(Record lRecordTipoDoc) {
				...
				String idTipoDocumento = lRecordTipoDoc != null ? lRecordTipoDoc.getAttribute("idTipoDocumento")
						: null;
				String descTipoDocumento = lRecordTipoDoc != null ? lRecordTipoDoc.getAttribute("descTipoDocumento")
						: null;
				Boolean flgTipoDocConVie = lRecordTipoDoc != null
						? lRecordTipoDoc.getAttributeAsBoolean("flgTipoDocConVie")
						: null;
				String siglaPraticaSuSistUfficioRichiedente = lRecordTipoDoc != null
						? lRecordTipoDoc.getAttribute("siglaPraticaSuSistUfficioRichiedente")
						: null;
				String idNodoDefaultRicercaAtti = lRecordTipoDoc != null
						? lRecordTipoDoc.getAttribute("idNodoDefaultRicercaAtti")
						: null;
				Map<String, Object> initialValues = new HashMap<String, Object>();
				
				initialValues.put("listaAltreVie", listaIndirizziRiferimento);
				initialValues.put("listaMittenti", listaMittenti);
				initialValues.put("idEmailArrivo", idEmail);
				initialValues.put("mezzoTrasmissione", "PEC");
				...
				RichiestaAccessoAttiDetail lRichiestaAccessoAttiDetail = new RichiestaAccessoAttiDetail(nomeEntita) {
					...
					public void afterProtocollaRichAccessoAttiSUECallback() {
						if(richAccessoAttiSUEmodalWindow != null) {
							richAccessoAttiSUEmodalWindow.hide();																	
						}
						reloadDetail();
						ricaricaPagina = true;
					}
				...
				richAccessoAttiSUEmodalWindow = Layout.addModalWindow("protocolla_accesso_atti_SUE", "Protocolla accesso atti SUE", "", lRichiestaAccessoAttiDetail);
			}
		});
	}
										...
										public void execute(final Record object) {
											if (object.getAttribute("idUd") != null && !object.getAttribute("idUd").trim().equals("")) {
												SC.ask(I18NUtil.getMessages().postaElettronica__list_copiaMailGiaProtocollataField(), new BooleanCallback() {
													...
													public void execute(Boolean value) {
														if (value) {
															manageModificaDatiRepertorioButtonClick(object.getAttribute("idUd"), destinatariEAccount, idTipoDoc, nomeTipoDoc, repertorio, lRecord);
														} else {
															SC.ask(I18NUtil.getMessages().postaElettronica__list_askChiusuraMailField(), new BooleanCallback() {
												...
												manageRegistraRepertorioCallback(object, destinatariEAccount, idTipoDoc, nomeTipoDoc, repertorio, false, null);
											}
										}
									});
								} else {
	...
	protected void manageModificaDatiRepertorioButtonClick(String idUd, final String destinatariEAccount, final String idTipoDoc, final String nomeTipoDoc, final String repertorio, final Record recordMail) {
		markForRedraw();
		manageLoadDetailIdUdMail(idUd, new DSCallback() {
			...
			public void execute(DSResponse response, Object rawData, DSRequest request) {
				if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
					final Record detailRecord = response.getData()[0];
					manageRegistraRepertorioCallback(detailRecord, destinatariEAccount, idTipoDoc, nomeTipoDoc, repertorio, true, recordMail);
				}
			}
	...
	private void manageRegistraRepertorioCallback(Record lRecord, String destinatariEAccount,

	...
	protected void createMittentiItem() {
		...
		mittentiItem = new MittenteProtUscitaItem() {
			...
			public boolean hasDefaultValue() {
				...
				if(AurigaLayout.getUoCollegateUtenteValueMap().size() == 1 && isRequiredDetailSectionMittenti()) {
					return true;
				}
				return AurigaLayout.getParametroDBAsBoolean("PREIMP_UO_COME_MITT_PROT_UI") && (AurigaLayout.getUoProtocollantiValueMap().size() == 1)
			...
			public Record getDefaultRecord() {
				...
				if (AurigaLayout.getUoProtocollantiValueMap().size() == 1) {
					String idUoSoggetto = AurigaLayout.getUoProtocollantiValueMap().keySet().toArray(new String[1])[0];
					String descrizione = AurigaLayout.getUoProtocollantiValueMap().get(idUoSoggetto);
		...
		if (mode == null || mode.equals("new") || mode.equals("edit")) {	
			if(destinatariItem instanceof DestinatarioProtUscitaItem) {
				if (((DestinatarioProtUscitaItem)destinatariItem).isAttivoDestEsternoUnicoCartaceoProt()) {				
					...
					if(destinatariItem != null && destinatariItem.getVisible()) {

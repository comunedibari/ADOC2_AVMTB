...
public class InvioMailFormNew extends DynamicForm {
	...
	public InvioMailFormNew(TipologiaMail tipologiaMail, String tipoRel, ValuesManager vm) {
		...
		CustomValidator lCustomValidator = getValidatorEmail();
		...
		createDestinatariPrimari(lCustomValidator);
		...
		createDestinaratiCC(lCustomValidator);
		...
		createDestinaratiCCN(lCustomValidator);
...
//			}
	...
	private boolean isMittentePec() {
		if (lSelectItemMittente != null && lSelectItemMittente.getDisplayValue() != null 
			&& !"".equals(lSelectItemMittente.getDisplayValue()) && lSelectItemMittente.getDisplayValue().contains("PEC") ) {
			return true;
		} else {
			return false;
		}

...
//				String descTipoDocDefault = AurigaLayout.getImpostazioniDocumento("descTipoDocumentoProtInterna");
				...
				public void execute(DSResponse response, Object rawData, DSRequest request) {
					...
					if (response.getData().length == 0) {
						...
						if (required && response.getData().length == 1) {
							...
							lRecord.setAttribute("flgTipoProtModulo", response.getData()[0].getAttributeAsBoolean("flgTipoProtModulo"));							
						}
						if (callback != null) {
			...
			if (AurigaLayout.getImpostazioniDocumentoAsBoolean("skipSceltaTipologiaRegistroFatture")) {
				...
				AurigaLayout.apriSceltaTipoDocPopup(false, idTipoDocDefault, null, null, new ServiceCallback<Record>() {
					...
					public void execute(Record lRecordTipoDoc) {
						...
						String idTipoDoc = lRecordTipoDoc != null ? lRecordTipoDoc.getAttribute("idTipoDocumento")
								: null;
						String nomeTipoDoc = lRecordTipoDoc != null ? lRecordTipoDoc.getAttribute("descTipoDocumento")
								: null;
						Map<String, Object> initialValues = new HashMap<String, Object>();
						if (idTipoDoc != null && !"".equals(idTipoDoc)) {
							initialValues.put("tipoDocumento", idTipoDoc);
			...
			if (AurigaLayout.getImpostazioniDocumentoAsBoolean("skipSceltaTipologiaBozza")) {
				...
				Boolean flgOggettoNonObblig = AurigaLayout
						.getImpostazioniDocumentoAsBoolean("flgOggettoNonObbligBozza");
				Map<String, Object> initialValues = new HashMap<String, Object>();
				if (idTipoDoc != null && !"".equals(idTipoDoc)) {
					initialValues.put("tipoDocumento", idTipoDoc);
					initialValues.put("nomeTipoDocumento", nomeTipoDoc);
		...
		// GIUNTA
			...
			AurigaLayout.apriSceltaSedutaOrganiCollegiali("giunta","CONVOCAZIONE", new ServiceCallback<Record>() {
				...
				public void execute(Record lRecord) {
					...
					Record getRercord = new Record();
					getRercord.setAttribute("idSeduta", lRecord.getAttribute("idSeduta") != null ? lRecord.getAttribute("idSeduta") : null);
					getRercord.setAttribute("organoCollegiale", "giunta");
					GWTRestService<Record, Record> lGWTRestDataSource = new GWTRestService<Record, Record>("ConvocazioneSedutaDataSource");
					lGWTRestDataSource.call(getRercord, new ServiceCallback<Record>() {
	...
	public void apriRepertorioDetail(final String nomeEntita, final String repertorio, final String idTipoDoc,
			final String nomeTipoDoc) {
	...
	public GWTRestDataSource getPreferenceDataSource() {
		return new GWTRestDataSource("AurigaPreferenceDataSource", "prefName", FieldType.TEXT);
	}

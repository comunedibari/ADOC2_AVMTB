...
public class DettaglioPraticaLayout extends HLayout {
	...
	protected HLayout buttonAnteprimaLayout;
	...
	public DettaglioPraticaLayout(Record record, ModalWindow window) {
		...
		buttonAnteprimaLayout = new HLayout();
		buttonAnteprimaLayout.setWidth(5);
		buttonAnteprimaLayout.setMembersMargin(2);
		buttonAnteprimaLayout.setOverflow(Overflow.VISIBLE);
		buttonAnteprimaLayout.setVisibility(Visibility.HIDDEN);
...
//		layoutSX.setPadding(5);
		...
		tabGestisciIter = new Tab("<b>Gestisci iter</b>");
...
//			if (!ProtocollazioneDetailAtti.isAbilToSelUffPropEsteso() && AurigaLayout.getUoProtocollantiValueMap().size() == 1) {
//				desUoProponente = AurigaLayout.getUoProtocollantiValueMap().values().toArray(new String[1])[0];
//			}
//		}
	...
	private Button creaVoce(final Record record) {
		...
		button.addHoverHandler(new HoverHandler() {
			...
			public void onHover(HoverEvent event) {
				button.setPrompt(getPromptIconaVoceMenu(flgFatta, flgEsito, flgToDo));
			}
		});
	...
	private Label buildLabelSottomenu(final Record recordSottomenu) {
		...
		label.addHoverHandler(new HoverHandler() {
			...
			@Override
			public void onHover(HoverEvent event) {
				label.setPrompt(getPromptIconaVoceMenu(flgFatta, flgEsito, flgToDo));
			}
		});
		if (flgDatiVisibili != null && "1".equals(flgDatiVisibili)) {
			...
			public void execute(Record lRecordPratica) {
				...
				if (!isInBozza) {
					...
					if (idDefProcFlow != null && !"".equals(idDefProcFlow)) {
						Label btnGrafo = new Label(
								"<div style='margin-top:8px; margin-right:5px; cursor:hand;'><img src=\"images/pratiche/task/grafico_del_flusso.png\" height=\"24\" width=\"24\"/></div>");
					...
					if (idProcess != null && !"".equals(idProcess)) {
						Label btnTree = new Label(
								"<div style='margin-top:8px; margin-right:5px; cursor:hand;'><img src=\"images/pratiche/task/iter_svolto.png\" height=\"24\" width=\"24\" /></div>");
					...
					if(abilitaCallApplTitoliEdilizi) {
						Label btnImportFromOnlyOne = new Label(
								"<div style='margin-top:8px; margin-right:5px; cursor:hand;'><img src=\"images/pratiche/task/import.png\" height=\"24\" width=\"24\" /></div>");
				...
				if (isFrontOffice()) {
					Label btnNote = new Label(
							"<div style='margin-top:8px; margin-right:5px; cursor:hand;'><img src=\"images/pratiche/task/note.png\" height=\"24\" width=\"24\" /></div>");
					...
					Label btnUltimaAttivita = new Label(
							"<div style='margin-top:8px; margin-right:5px; cursor:hand;'><img src=\"images/pratiche/task/note.png\" height=\"24\" width=\"24\" /></div>");
				...
				buttonAnteprimaLayout.setMembers();
				buttonAnteprimaLayout.setVisibility(Visibility.HIDDEN);				
				buttonAnnulla.setVisibility(Visibility.HIDDEN);				
...
//							}
				...
				listaButtons.add(buttonAnteprimaLayout);
				...
				if(AurigaLayout.isAttivaNuovaPropostaAtto2Completa()) {
					canvas = new TaskNuovaPropostaAtto2CompletaDetail(nomeEntita, idProcess, nomeFlussoWF, processNameWF, idUd, record, instance) {
						...
						public void afterShow() {
							...
							List<Canvas> listaButtonAnteprimaLayoutMembers = new ArrayList<Canvas>();
							...
							if(getFlgSoloPreparazioneVersPubblicazione() && isEseguibile()) {								
								...
								listaButtonAnteprimaLayoutMembers.add(buttonAggiornaVersDaPubblicare);
...
//										});										
								...
								listaButtonAnteprimaLayoutMembers.add(buttonFrecciaAggiornaVersDaPubblicare);									
							} else {									
								...
								listaButtonAnteprimaLayoutMembers.add(buttonAnteprimaProvvedimento);								
							}
		
							if(idModAppendice != null && !"".equals(idModAppendice)) {											
								final FrontendButton buttonAnteprimaDatiSpesa = new FrontendButton("Anteprima movimenti contabili", "pratiche/task/buttons/anteprima_dati_spesa.png");
								...
								listaButtonAnteprimaLayoutMembers.add(buttonAnteprimaDatiSpesa);	
							} 
							...
							if(uriFilePrimario != null && !"".equals(uriFilePrimario) && infoFilePrimario != null && infoFilePrimario.isFirmato()) {
								...
								listaButtonAnteprimaLayoutMembers.add(buttonScaricaAttoFirmato);
							} else {								
									...
									public void onClick(ClickEvent event) {
										...
										if(hasDatiSensibili()) {											
											...
											scaricaFileUnioneVersIntegraleMenuItem.addClickHandler(new com.smartgwt.client.widgets.menu.events.ClickHandler() {
												...
												@Override
												public void onClick(MenuItemClickEvent event) {														
													anteprimaFileUnioneVersIntegrale(new ServiceCallback<Record>() {
											...
											scaricaFileUnioneVersOmissisMenuItem.addClickHandler(new com.smartgwt.client.widgets.menu.events.ClickHandler() {
												...
												@Override
												public void onClick(MenuItemClickEvent event) {														
													anteprimaFileUnioneVersXPubbl(new ServiceCallback<Record>() {
								...
								listaButtonAnteprimaLayoutMembers.add(buttonScaricaFile);							
							}
							...
							if (flgPresentiEmendamenti != null && flgPresentiEmendamenti) {																
								...
								listaButtonAnteprimaLayoutMembers.add(buttonListaEmendamenti);	
								...
								listaButtonAnteprimaLayoutMembers.add(buttonFrecciaApriEmendamentiWindow);							
							}
							
							buttonAnteprimaLayout.setMembers(listaButtonAnteprimaLayoutMembers.toArray(new Canvas[listaButtonAnteprimaLayoutMembers.size()]));							
							buttonAnteprimaLayout.setVisibility(Visibility.VISIBLE);
						}	
					};		
				} else {
						...
						public void afterShow() {
							...
							if(idUdPubblPost != null && !"".equals(idUdPubblPost)) {
								...
								if(idUd != null && setIdUdPubblicaPost.contains(idUd)) {
									...
									listaButtonAnteprimaLayoutMembers.add(buttonPubblicaPost);
								}
							}
				...
				if(AurigaLayout.isAttivaNuovaPropostaAtto2Completa()) {
					canvas = new TaskRevocaNuovaPropostaAtto2CompletaDetail(nomeEntita, idProcess, nomeFlussoWF, processNameWF, idUd, record, instance) {
						...
						public void afterShow() {
							...
							buttonAnteprimaLayout.setMembers(buttonAnteprimaProvvedimento);							
							buttonAnteprimaLayout.setVisibility(Visibility.VISIBLE);
						}	
					};			
				} else {
			...
			canvas = new TaskNuovaRichiestaAccessoAttiDetail(nomeEntita, idProcess, nomeFlussoWF, processNameWF, idUd, record, instance) {
				...
				public void afterShow() {
					...
					if (record.getAttributeAsBoolean("abilAbilitaAppuntamentoDaAgenda")) {
							...
							public void onClick(ClickEvent event) {
								...
								manageAzioneSuRichAccessoAttiSingola("ABILITA_APPUNTAMENTO_DA_AGENDA", new ServiceCallback<Record>() {
									...
									public void execute(Record record) {
										...
										if(success) {
											// nascondo tutto buttonAnteprimaLayout perchè dentro c'è solo abilitaAppuntamentoDaAgendaButton
											buttonAnteprimaLayout.setVisibility(Visibility.HIDDEN);
										}
									}
								});
						...
						buttonAnteprimaLayout.setMembers(abilitaAppuntamentoDaAgendaButton);
						buttonAnteprimaLayout.setVisibility(Visibility.VISIBLE);
					}
				}
			};
			...
			canvas = new TaskPropostaOrganigrammaDetail(nomeEntita, null, idProcess, nomeFlussoWF, processNameWF, idUd, record, instance) {
				...
				public void afterShow() {
					...
					if(isEseguibile()) {
						...
						buttonAnteprimaLayout.setMembers(apriEditorOrganigrammaButton, buttonAnteprimaProposta);
						
					} else {
						buttonAnteprimaLayout.setMembers(buttonAnteprimaProposta);						
					}
	...
	public String getPromptIconaVoceMenu(String flgFatta, String flgEsito, String flgToDo) {
		if (flgFatta != null) {
			if ("2".equals(flgFatta)) {
				if (flgEsito != null) {
					if ("OK".equalsIgnoreCase(flgEsito)) {
	...
	public HashMap<String, Record> getMappaMenu() {
		return mappaMenu;
	}

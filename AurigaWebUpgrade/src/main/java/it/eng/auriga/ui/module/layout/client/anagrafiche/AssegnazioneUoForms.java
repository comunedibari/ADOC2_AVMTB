...
//				}
	...
	private void disegnaAssegnazioneUoCollegateUtente() {
		LinkedHashMap<String, String> mappaUoCollegate = AurigaLayout.getUoProtocollantiValueMap();
	...
	private void checkForzaSelectUO() {
		...
		if (AurigaLayout.getUoProtocollantiValueMap().size() == 1) {
			...
			Set<String> listaIdUo = AurigaLayout.getUoProtocollantiValueMap().keySet();
			Iterator<String> iterator = listaIdUo.iterator();
			// Sono sicuro che ce ne sia una
			String idUo = iterator.next();
			String descrizioneUO = AurigaLayout.getUoProtocollantiValueMap().get(idUo);
	...
	private boolean showUoCollegateItem() {
		return (mode != null && ("new".equals(mode) || "edit".equals(mode))) && !isAbilInserireModificareSoggInQualsiasiUo()
				&& AurigaLayout.getUoProtocollantiValueMap().size() > 1;
	}
	...
	private boolean showLabelUoCorrenteItem() {
		return (mode != null && "view".equals(mode)) || (!isAbilInserireModificareSoggInQualsiasiUo() && AurigaLayout.getUoProtocollantiValueMap().size() == 1);
	}
		...
		if(mode != null && ("new".equals(mode) || "edit".equals(mode))) {
			...
			if (isPartizionamentoRubricaAbilitato()) {
				...
				if (descrizione == null || "".equalsIgnoreCase(descrizione)) {
					descrizione = AurigaLayout.getUoProtocollantiValueMap().get("UO"+idUoSoggetto);
				}
				if (descrizione == null || "".equalsIgnoreCase(descrizione)) {

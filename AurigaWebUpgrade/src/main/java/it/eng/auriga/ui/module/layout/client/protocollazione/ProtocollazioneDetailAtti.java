...
public class ProtocollazioneDetailAtti extends ProtocollazioneDetail {
	...
	public static boolean isAttivaPropostaAtto2() {
	...
	public boolean isVecchiaPropostaAtto2Milano() {
		...
		return isAttivaPropostaAtto2() && !AurigaLayout.getParametroDBAsBoolean("ATTIVA_NUOVA_PROPOSTA_ATTO_2") && !AurigaLayout.getParametroDBAsBoolean("GESTIONE_ATTI_COMPLETA");
	}
	...
	public boolean showUoProtocollanteSelectItem() {
		return /*isAbilToProt() && */(AurigaLayout.getUoCollegateUtenteValueMap().size() > 1 || isAbilToSelUffPropEsteso());
	}
...
//	}
	...
	public boolean showDetailSectionResponsabileProc() {
		return !isAttivaPropostaAtto2() && AurigaLayout.getParametroDBAsBoolean("SHOW_RESP_PROC_IN_ATTI");
	}
				...
				if (uoProponente == null || "".equals(uoProponente)) {
					if (uoProtocollanteSelectItem != null) {
						if (uoProtocollanteSelectItem.getValueAsString() != null && !"".equals(uoProtocollanteSelectItem.getValueAsString())) {
							...
							uoProponente = AurigaLayout.getUoProtocollantiValueMap().keySet().toArray(new String[1])[0];
						}
	...
	public String getTitleNomeFilePrimario() {
		return isAttivaPropostaAtto2() ? "Dispositivo" : I18NUtil.getMessages().protocollazione_detail_nomeFilePrimarioItem_title();
	}

	@Override
	public int getWidthNomeFilePrimario() {
		return isAttivaPropostaAtto2() ? 250 : 320;
	}
...
//		return hasOneOrMoreFlgNoPubblChecked();
	...
	public VLayout getLayoutAssegnazioneEClassificazione() {
		...
		detailSectionAssegnazione.setVisible(!isAttivaPropostaAtto2());
		layoutAssegnazioneEClassificazione.addMember(detailSectionAssegnazione);

		createDetailSectionCondivisione();
		detailSectionCondivisione.setVisible(!isAttivaPropostaAtto2());
		...
		if (AurigaLayout.getParametroDBAsBoolean("ATTIVA_FOLDER_CUSTOM")) {
			...
			detailSectionFolderCustom.setVisible(!isAttivaPropostaAtto2());
	...
	public String getTitleDetailSectionAltreUoCoinvolte() {
		return isAttivaPropostaAtto2() ? "Altre strutture coinvolte" : "Altre U.O. proponenti";
	}

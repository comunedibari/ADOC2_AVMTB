...
public class AzioneApposizionePopup extends ModalWindow {
	 ...
	 * @param filesAndUd
	 */
	protected void manageFirmaMassivaCallBack(final Map<String, Record> files, final Record[] filesAndUd,
			final ServiceCallback<Record> callbackManageFirma) {
			...
			public void execute(DSResponse response, Object rawData, DSRequest request) {
				...
				if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
					...
					if (lRecordsInError.length > 0) {
							...
							if (documentiInError.size() == files.size()) {
								if (documentiInError.size() == 1) {
									Layout.addMessage(new MessageBean(
											"Il documento selezionato non è stato firmato correttamente", "",
											MessageType.ERROR));
								} else {
									Layout.addMessage(new MessageBean(
											"Tutti i documenti selezionati non sono stati firmati correttamente", "",
											MessageType.ERROR));
								}
							} else {
								String message = null;
								if (documentiInError.size() == 1) {
									message = "Il documento " + documentiInError.get(0)
											+ " non è stato firmato correttamente";
								} else {
									message = "I documenti ";
									boolean first = true;
						...
						if (recordSelezionatiPerFirma.getLength() == 1) {
							Layout.addMessage(new MessageBean("Il documento selezionato è stato firmato con successo",
									"", MessageType.INFO));
						} else {
							Layout.addMessage(new MessageBean("I documenti selezionati sono stati firmati con successo",
									"", MessageType.INFO));
						}
					...
					Layout.addMessage(
							new MessageBean("Si è verificato un errore durante la firma", "", MessageType.ERROR));
				}
			}
		}, new DSRequest());
	}
			...
			public void execute(DSResponse response, Object rawData, DSRequest request) {
				...
				if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
					...
					if (lRecordsInError.length > 0 && lRecordsInError.length == lRecords.length) {
						...
						Layout.addMessage(new MessageBean("Si è verificato un errore durante la firma del documento",
								"", MessageType.ERROR));
						return;
						...
						Layout.addMessage(
								new MessageBean("Il documento è stato firmato con successo", "", MessageType.INFO));

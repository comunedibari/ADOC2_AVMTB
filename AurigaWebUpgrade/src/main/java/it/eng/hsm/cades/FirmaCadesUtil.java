...
public class FirmaCadesUtil {
	...
	public static byte[] generaP7mVerticale(byte[] fileDaFirmare, X509Certificate userCert, Hsm clientHsm) throws HsmClientSignatureException {
		
		if(clientHsm.getHsmConfig().getHsmType().equals(HsmType.PKBOX)){
	...
	public static List<byte[]> generaP7mVerticalePkBox(List<byte[]> listaFileDaFirmare, X509Certificate userCert, Hsm clientHsm) throws HsmClientSignatureException {
	...
	public static byte[] generaP7mCongiunta(byte[] fileDaFirmare, X509Certificate userCert, Hsm clientHsm) throws HsmClientSignatureException {
		try {
			ByteArrayOutputStream bout = new ByteArrayOutputStream();
			java.security.Security.addProvider(new org.bouncycastle.jce.provider.BouncyCastleProvider());
	...
	public static List<byte[]> generaP7mCongiuntaPkBox(List<byte[]> listaFileDaFirmare, X509Certificate userCert, Hsm clientHsm) throws Exception {
					...
					for (int i = 0; i < listHashResponseBean.size(); i++) {
						try {
							...
							if (listHashResponseBean.get(i).getMessage().getStatus().equals(ResponseStatus.OK)) {
								...
								logger.error(
										"Errore nella firma per il file numero " + i + " Errore: " + listHashResponseBean.get(i).getMessage().getDescription());
								throw new HsmClientSignatureException("Errore nella firma: " + listHashResponseBean.get(i).getMessage().getDescription());
							}
						} catch (Exception e) {
							logger.error("Errore nella firma", e);
	...
	public static byte[] getDigest(File fileDaFirmare) throws Exception {
		MessageDigest md;
		md = MessageDigest.getInstance("SHA256");

		byte[] buf = new byte[65664];

		ByteArrayOutputStream _fileOut = new ByteArrayOutputStream();
		InputStream fis = new FileInputStream(fileDaFirmare);
		int nRead = 0;
		while ((nRead = fis.read(buf)) > 0) {
			_fileOut.write(buf, 0, nRead);
			md.update(buf, 0, nRead);
		}
		fis.close();
		_fileOut.close();

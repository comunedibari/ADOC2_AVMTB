...
public class SignerThread extends Thread {
	...
	public void run() {
		...
		for (String path : pathProviders) {
			...
			try {
						...
						if (authorizationInfos.isValid()) {
							...
							if (hashfilebean != null && !hashfilebean.isEmpty()) {
									...
									if (x500Subject != null) {
										...
										if (cns != null && cns.length > 0) {
											...
											if (cn != null && cn.getFirst() != null) {
												...
												// } else {
												// logger.info("Il file è stato già firmato dal CN subject certificato " );
												// panelSign.getTextArea().append("Il file " + hash.getFileName() + " è stato già firmato dal CN
												// subject
												// certificato.\n");
												// }
											}
								...
								if (esitoFirmaComplessiva) {
									...
									// panelSign.showMessageDialog( Messages.getMessage( MessageKeys.MSG_END_WARNING ), "",
									// JOptionPane.INFORMATION_MESSAGE );
									// }
									// else {
									// panelSign.showMessageDialog( Messages.getMessage( MessageKeys.MSG_OPSUCCESS ), "",
									// JOptionPane.INFORMATION_MESSAGE );
									// }
								...
								if (fileBeanList != null) {
									...
									for (FileBean bean : fileBeanList) {
										...
										if (counterSign) {
											...
											boolean esitoFirma = signer.firma(bean, pvc, provider, timemark, false, bean.isFirmaCongiuntaRequired(),
				...
				logger.info("Errore nel recupero certificati "/*, e*/);
				...
				if (exceptionMessage.equalsIgnoreCase(Messages.getMessage(MessageKeys.MSG_FIRMAMARCA_ERROR_PINLOCKED))) {
					...
					//Se da parametro di input � stato impostato che dev'essere saltato il break 
					//if(breakOnWrongPin)
					//{
						// "Pin non corretto!";
						panelSign.gestisciEccezione(exceptionMessage);
	...
	private void closeConnectionSunProvider() {
		...
		try {
			...
			logger.error("Errore durante la chiusura della connessione con sunProvider: " + e.getMessage());
			e.printStackTrace();
		}
	}

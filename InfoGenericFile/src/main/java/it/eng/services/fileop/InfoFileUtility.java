...
//			firmatarioExt.setQcStatements(infoFirmatario.getCertificato().getQcStatements().getQcStatement()
//					.toArray(new String[infoFirmatario.getCertificato().getQcStatements().getQcStatement().size()]));
...
//		firmatarioExt.setDataFirma(info.getSigningTime() != null ? sdf.format(info.getSigningTime().toGregorianCalendar().getTime()) : null);
		...
		firmatarioExt.setStatoCertificato(infoFirmatario.getVerificationStatus().equals(VerificationStatusType.OK) ? "Certificato credibile"
				: "Certificato non credibile");
		firmatarioExt
				.setCertExpiration(infoFirmatario.getSigVerifyResult().getCertExpirationResult().getVerificationStatus().equals(VerificationStatusType.OK) ? "Il certificato è in corso di validità"
	...
	private String getInfoFirma(DocumentConfiguration lDocumentConfiguration, SignatureVerifyOptionBean lSignatureVerifyOptionBean) {
		return lDocumentConfiguration.toString() + "/n" + lSignatureVerifyOptionBean.toString();
	}
	
	private String getInfoFirmaControFirma(DocumentConfiguration lDocumentConfiguration, SignatureVerifyOptionBean lSignatureVerifyOptionBean, String firmatarioPrimaFirma, Date dataFirma) {

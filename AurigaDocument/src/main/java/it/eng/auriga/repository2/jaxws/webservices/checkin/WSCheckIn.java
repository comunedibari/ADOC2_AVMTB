	...
	private String salvaAllegato(AurigaLoginBean loginBean, Connection conn, String nroVersioneEstratta, RebuildedFile rebuildedFileIn,
		...
		try {
			...
			if (!nroVersioneEstratta.trim().equals("") && !nroVersioneEstratta.trim().equals("0")) {
				 ...
				 * try {
				 * 
				 * GestioneDocumenti servizio = new GestioneDocumenti(); CreaModDocumentoOutBean servizioOut = new CreaModDocumentoOutBean(); servizioOut =
				 * servizio.addUpdDocsWS(loginBean, flgTipoFile, idUd, idDocFileDecimal, lAllegatiBean);
				 * 
				 * 
				 * // Se il servizio e' andato in errore restituisco il messaggio di errore if(StringUtils.isNotBlank(servizioOut.getDefaultMessage())) {
				 * returnMess += (unaStepGiaFallita)?", "+ servizioOut.getDefaultMessage() : " " + servizioOut.getDefaultMessage(); }
				 * 
				 * //popolo un oggetto VersioneElettronicaNonCaricata per tener conto del fallimento VersioneElettronicaNonCaricata venc = new
				 * VersioneElettronicaNonCaricata();
				 * 
				 * // Se la gestione dei file e' andata in errore restituisco il messaggio di errore StringBuffer lStringBuffer = new StringBuffer();
				 * 
				 * int key = 0; if (servizioOut.getFileInErrors()!=null && servizioOut.getFileInErrors().size()>0){ for (String lStrFileInError :
				 * servizioOut.getFileInErrors().values()){
				 * 
				 * lStringBuffer.append("; " + lStrFileInError);
				 * 
				 * //setto l'indice String sIndex = servizioOut.getFileInErrors().get(key); if (sIndex != null && !sIndex.equalsIgnoreCase(""))
				 * venc.setNroAttachmentAssociato(new Integer(sIndex));
				 * 
				 * //setto il nome file venc.setNomeFile(lStrFileInError);
				 * 
				 * //aggiungo alla lista dei fallimenti versioniNonCaricate.add(venc);
				 * 
				 * key++; } }
				 * 
				 * if (lStringBuffer != null && lStringBuffer.length()>0 && !lStringBuffer.toString().equalsIgnoreCase("")) returnMess +=
				 * (unaStepGiaFallita)?", "+ lStringBuffer : " " + lStringBuffer; } catch (Exception ve) { String mess =
				 * "------> Fallita la checkIn per il doc " + idDoc; returnMess += (unaStepGiaFallita)?", "+ mess : " " + mess; unaStepGiaFallita = true;
				 * aLogger.debug(mess + " - " + ve.getMessage(), ve); }
				 */

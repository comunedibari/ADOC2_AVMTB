		...
		if (result.isInError()) {
			...
			if (pFilePrimarioBean != null) {			
				...
				if (pFilePrimarioBean.getFileOmissis() != null) {					
					...
					final MarktokenusageImpl lMarktokenusageOmissis = new MarktokenusageImpl();
					DmpkLoginMarktokenusageBean lMarktokenusageBeanOmissis = new DmpkLoginMarktokenusageBean();
					if (pAurigaLoginBean.getToken() != null)
						lMarktokenusageBeanOmissis.setCodidconnectiontokenin(pAurigaLoginBean.getToken());
					lMarktokenusageBeanOmissis.setFlgautocommitin(0);
					lMarktokenusageOmissis.setBean(lMarktokenusageBeanOmissis);
					session.doWork(new Work() {

						@Override
						public void execute(Connection paramConnection) throws SQLException {
							lMarktokenusageOmissis.execute(paramConnection);
						}
					});
				...
				if (pAllegatiBean.getIsNullOmissis() != null) {
					for (int i = 0; i < pAllegatiBean.getIsNullOmissis().size(); i++) {	
						if (!pAllegatiBean.getIsNullOmissis().get(i)) {						
							...
							final MarktokenusageImpl lMarktokenusageAllegatoOmissis = new MarktokenusageImpl();
							DmpkLoginMarktokenusageBean lMarktokenusageBeanAllegatoOmissis = new DmpkLoginMarktokenusageBean();
							if (pAurigaLoginBean.getToken() != null)
								lMarktokenusageBeanAllegatoOmissis.setCodidconnectiontokenin(pAurigaLoginBean.getToken());
							lMarktokenusageBeanAllegatoOmissis.setFlgautocommitin(0);
							lMarktokenusageAllegatoOmissis.setBean(lMarktokenusageBeanAllegatoOmissis);
							session.doWork(new Work() {
		
								@Override
								public void execute(Connection paramConnection) throws SQLException {
									lMarktokenusageAllegatoOmissis.execute(paramConnection);
								}
							});
			...
			if (pAllegatiBean != null) {
				...
				if (pAllegatiBean.getIdDocumentoOmissis() != null) {
					for (int i = 0; i < pAllegatiBean.getIdDocumentoOmissis().size(); i++) {
						...
						lAllegatoStoreBeanOmissis.setFlgVersConOmissis("1");
						lAllegatoStoreBeanOmissis.setIdDocVersIntegrale(idDocAllegato != null ? String.valueOf(idDocAllegato.longValue()) : null);				
						String attributiAllegatoOmissis = lXmlUtilitySerializer.bindXmlCompleta(lAllegatoStoreBeanOmissis);						
						BigDecimal idDocAllegatoOmissis = pAllegatiBean.getIdDocumentoOmissis().get(i);
						if (idDocAllegatoOmissis != null) {
							//TODO ha senso fare la Upddocud per la versione con omissis?
//							DmpkCoreUpddocudBean lUpddocudBeanAllegatoOmissis = new DmpkCoreUpddocudBean();
//							lUpddocudBeanAllegatoOmissis.setCodidconnectiontokenin(pAurigaLoginBean.getToken());
//							lUpddocudBeanAllegatoOmissis.setIduserlavoroin(getIdUserLavoro(pAurigaLoginBean));
//							lUpddocudBeanAllegatoOmissis.setFlgtipotargetin("D");
//							lUpddocudBeanAllegatoOmissis.setIduddocin(idDocAllegatoOmissis);
//							lUpddocudBeanAllegatoOmissis.setAttributiuddocxmlin(attributiAllegatoOmissis);
//							final UpddocudImpl storeAllegatoOmissis = new UpddocudImpl();
//							storeAllegatoOmissis.setBean(lUpddocudBeanAllegatoOmissis);
//							lLoginService.login(pAurigaLoginBean);
//							session.doWork(new Work() {
//
//								@Override
...
//								}
//							});
//							StoreResultBean<DmpkCoreUpddocudBean> resultAllegatoOmissis = new StoreResultBean<DmpkCoreUpddocudBean>();
//							AnalyzeResult.analyze(lUpddocudBeanAllegatoOmissis, resultAllegatoOmissis);
//							resultAllegatoOmissis.setResultBean(lUpddocudBeanAllegatoOmissis);
//							final MarktokenusageImpl lMarktokenusageAllegatoOmissis = new MarktokenusageImpl();
//							DmpkLoginMarktokenusageBean lMarktokenusageBeanAllegatoOmissis = new DmpkLoginMarktokenusageBean();
//							if (pAurigaLoginBean.getToken() != null)
//								lMarktokenusageBeanAllegatoOmissis.setCodidconnectiontokenin(pAurigaLoginBean.getToken());
...
//							});
//							if (resultAllegatoOmissis.isInError()) {
//								mLogger.debug(resultAllegatoOmissis.getDefaultMessage());
//								mLogger.debug(resultAllegatoOmissis.getErrorContext());
//								mLogger.debug(resultAllegatoOmissis.getErrorCode());
//								throw new StoreException(resultAllegatoOmissis);
//							} else {
...
//							}							
	...
	public CreaModDocumentoOutBean addUpdDocsWS(AurigaLoginBean pAurigaLoginBean, String flgTipoFile, String pIdUd, BigDecimal pIdDocFile,
			AllegatiBean pAllegatiBean) throws JAXBException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		CreaModDocumentoOutBean lModificaDocumentoOutBean = new CreaModDocumentoOutBean();
		...
		String pIdDocPrimario = lDocumentoXmlOutBean.getIdDocPrimario();
		BigDecimal idDocPrimario = (pIdDocPrimario != null) ? new BigDecimal(pIdDocPrimario) : null;
		...
		try {
			...
			Transaction lTransaction = session.beginTransaction();
			try {
				manageAddUpdDocsWS(pAurigaLoginBean, flgTipoFile, idDocPrimario, pAllegatiBean, idUd, pIdDocFile, lDocumentoXmlOutBean, versioni,
			...
			// Parte di versionamento
			Map<String, String> fileErrors = new HashMap<String, String>();
			fileErrors.putAll(aggiungiFilesWS(pAurigaLoginBean, versioni));
	...
	public BigDecimal leggiIdDocPrimarioWSInTransaction(Locale localeIn ,AurigaLoginBean loginBeanIn, String idUdIn,  Session sessionIn) throws Exception{
		...
		try {		
				...
				final MarktokenusageImpl lMarktokenusage = new MarktokenusageImpl();
				DmpkLoginMarktokenusageBean lMarktokenusageBean = new DmpkLoginMarktokenusageBean();
				if (loginBeanIn.getToken() != null)
					lMarktokenusageBean.setCodidconnectiontokenin(loginBeanIn.getToken());
				lMarktokenusageBean.setFlgautocommitin(0);
				lMarktokenusage.setBean(lMarktokenusageBean);

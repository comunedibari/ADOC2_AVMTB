...
public class CodeDetectorCtrl extends AbstractFileController {
	...
	public boolean execute(InputFileBean input, AbstractInputOperationType customInput, OutputOperations output, String requestKey) {
		...
		log.debug("Metodo execute di CodeDetectorCtrl ");

		// leggo il file in ingresso che è lo sbustato o quello originale se non è una firma
		...
		log.info("Elaborazione file " + file);
		...
		try {
			detectorConfig = setUpConfig(configProperties);
		} catch (Exception e1) {
	...
	protected DynamicCodeDetectorConfig setUpConfig(Properties config) throws DynamicCodeDetectorException, SAXException, IOException {
		...
		String pdfFlag = config.getProperty(PDF_KEY, "true");
		...
		for (String key : keyset) {
			...
			for (DynamicCodeDetector detector : detectors) {
				...
				if (("OfficeMacroDetector".equals(detectorName) && "true".equalsIgnoreCase(msOfficeFlag))
						...
						|| ("PDFExecutableDetector".equals(detectorName) && "true".equalsIgnoreCase(pdfFlag)))

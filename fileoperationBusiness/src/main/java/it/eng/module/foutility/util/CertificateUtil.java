	...
	public static List<String> getQCStatements(X509Certificate certificato) {
		...
		for (int i = 0; i < seq.size(); i++) {
			...
			if (QCStatement.id_etsi_qcs_QcCompliance.equals(qcs.getStatementId())) {
				...
				qCStatements.add("Certificato qualificato (OID: " + qcs.getStatementId() + ")");
				...
				qCStatements.add("Dispositivo sicuro (OID: " + qcs.getStatementId() + ")");
			} else if (QCStatement.id_etsi_qcs_RetentionPeriod.equals(qcs.getStatementId())) {
				qCStatements.add("Periodo conservazione informazioni relative alla emissione del certificato qualificato: " + qcs.getStatementInfo()
						+ " anni (OID: " + qcs.getStatementId() + ")");

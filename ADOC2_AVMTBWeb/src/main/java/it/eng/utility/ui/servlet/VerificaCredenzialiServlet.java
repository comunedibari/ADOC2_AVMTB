...
public class VerificaCredenzialiServlet {
	...
	public ResponseEntity<String> verifica(@RequestBody String json,HttpSession session, HttpServletRequest servletrequest, HttpServletResponse servletresponse, ModelMap model) throws Exception {
		...
		if (StringUtils.isNotEmpty(action) && action.equals("logout")){
			...
			//Recupero lo schema
			boolean login = makeLogin(username, password);
			//Faccio la login
			if (login) {
				...
				lLoginBean.setSchema(SpringAppContext.getContext().getBean(SchemaSelector.class).getSchemi().get(0).getName());
				lAurigaLoginBean.setSchema(SpringAppContext.getContext().getBean(SchemaSelector.class).getSchemi().get(0).getName());
	...
	private boolean makeLogin(String username, String password) {
		...
		mLogger.debug("Schema vale " + lSchemaSelector.getSchemi().get(0).getName());
		mLogger.debug("realPassword vale " + password);
		...
		lSchemaBean.setSchema(lSchemaSelector.getSchemi().get(0).getName());
		try {

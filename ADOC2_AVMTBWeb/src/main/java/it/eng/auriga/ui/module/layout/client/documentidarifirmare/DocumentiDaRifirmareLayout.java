...
public class DocumentiDaRifirmareLayout extends CustomLayout {
	...
	private void firmaFile(Record record) {
		...
		/*
		 * Metto come primo parametro per la chiamata di FirmaMultiplaHashWindow il valore null perchè corrisponde alla forzatura del caso degli atti (questo
		 * non è il caso e quindi non dev'essere settato tale valore)
		 */
		FirmaMultiplaHashWindow lFirmaMultiplaHashCongiuntaWindow = new FirmaMultiplaHashWindow(null, lMap) {
			...
			@Override
			public void firmaCallBack(Map<String, Record> filesCongiunta, String commonName) {
				manageFirmaCallBack(filesCongiunta, filesAndUd);
			}
		};
		lFirmaMultiplaHashCongiuntaWindow.show();
	}
...
//					recordToInsert.setAttribute("infoFileVerPreFirma", infoFile);
		...
		String customOperation = AurigaLayout.getParametroDBAsBoolean("ATTIVA_ARCHIVIAZIONE_DOCUMENTI_DA_RIFIRMARE") ? "aggiornaDocumentoFirmato"
				: "verificaDocumentoFirmato";

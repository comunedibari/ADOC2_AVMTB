...
public class MittenteProtInternaCanvas extends IndirizzoCanvas {
	...
	public void buildMainForm() {
		...
		// tipo
		tipoMittenteItem = new SelectItem("tipoMittente");
		...
		if (getAllowOnlyUnitaDiPersonale()) {
			...
			tipoMittenteItem.setAllowEmptyValue(true);
		}
...
//					lRecord.setAttribute("flgAssegnaAlMittente", false); // non posso settare il check a false altrimenti non viene caricato successivamente  
		...
		codRapidoMittenteItem.setWidth(80);
...
//				mDynamicForm.setValue("organigrammaMittente", record.getAttributeAsString("id"));
		...
		organigrammaMittenteItem.setWidth(400);
...
//				}				
		...
		denominazioneMittenteItem.setWidth(200);
		...
		codfiscaleMittenteItem.setWidth(120);
		...
		ImgButtonItem salvaInRubricaButton = new ImgButtonItem("salvainrubricaMittente", "buttons/saveIn.png", I18NUtil.getMessages()
...
//		opzioniInvioAssegnazioneButton.setAlwaysEnabled(true);
		...
		mDynamicForm.setFields(
			idMittenteItem, idSoggettoItem, idUoSoggettoItem, idUserSoggettoItem, idScrivaniaSoggettoItem, idAssegnatarioItem, flgSelXAssegnazioneItem, codRapidoChangedHiddenItem, tipoMittenteItem,
			iconaFlgInOrganigrammaItem, codRapidoMittenteItem, lookupRubricaButton, lookupOrganigrammaButton, organigrammaMittenteItem, denominazioneMittenteItem, cognomeMittenteItem, nomeMittenteItem,
			codfiscaleMittenteItem, cercaInRubricaButton, cercaInOrganigrammaButton, salvaInRubricaButton, flgAssegnaAlMittenteItem,
			opzioniInvioHiddenItem, opzioniInvioAssegnazioneButton
		);
...
//			mDynamicForm.setValue("flgAssegnaAlMittente", false); // non posso settare il check a false altrimenti non viene caricato successivamente  
	...
	public void setFormValuesFromRecordRubrica(Record record) {
		...
		if (tipoMittente != null) {
			...
			mDynamicForm.setValue("idMittente", idSoggetto);
	...
	public void setFormValuesFromRecordOrganigramma(Record record) {
		...
		if(showSelectOrganigramma()) {	
			...
			mDynamicForm.setValue("organigrammaMittente", tipo + idUoSvUt);
		}
		mDynamicForm.setValue("flgSelXAssegnazione", record.getAttribute("flgSelXAssegnazione"));		
		mDynamicForm.clearValue("flgAssegnaAlMittente");
		((MittenteProtItem) getItem()).manageChangedFlgAssegnaAlMittente(mDynamicForm.getValuesAsRecord());
	...
	public void editRecord(Record record) {
		showFlgAssegnaAlMittente = false;
		if(showSelectOrganigramma()) {			
			String tipoMittente = record.getAttribute("tipoMittente");
			if (record.getAttribute("organigrammaMittente") != null && !"".equals(record.getAttributeAsString("organigrammaMittente"))
				...
				if (tipoMittente != null && "UOI".equals(tipoMittente)) {
			...
			if (record.getAttribute("organigrammaMittente") != null && !"".equals(record.getAttributeAsString("organigrammaMittente"))
					&& tipoMittente != null	&& ("UOI".equals(tipoMittente) || "UP".equals(tipoMittente))) {
	...
	public String getFinalitaOrganigramma() {
		if (isInBozza() && ((MittenteProtItem) getItem()).getFlgSoloInOrganigramma()) {
			return "ASS";
		} else {
			return "MITTENTE_REG";
		}

...
import it.eng.auriga.ui.module.layout.server.gestioneAtti.bean.ValoreUoPropXmlBean;
	...
	public PaginatorBean<AttiBean> fetch(AdvancedCriteria criteria, Integer startRow, Integer endRow, List<OrderByBean> orderby) throws Exception {
		...
		if(criteria!=null && criteria.getCriteria()!=null){		
			for(Criterion crit : criteria.getCriteria()){
				if("idProcessType".equals(crit.getFieldName())) {
		...
		List<ValoreUoPropXmlBean> listValoreUoPropXmlBean = new ArrayList<ValoreUoPropXmlBean>();
		if (porzioneRubrica != null) {
			for (DestInvioNotifica item : porzioneRubrica) {
				
				ValoreUoPropXmlBean lValoreUoPropXmlBean = new ValoreUoPropXmlBean();
				lValoreUoPropXmlBean.setIdUOProponentiProposta(item.getId());
				listValoreUoPropXmlBean.add(lValoreUoPropXmlBean);
			}
			scCriteriAvanzati.setIdUOProponentiProposta(listValoreUoPropXmlBean);
		}
		...
		if (StringUtils.isNotEmpty(taskEffettuato)){			
			...
			for (int i = 0 ; i < stringaSplittata.length; i += 2 ){
				...
				lCriterioIterSvoltoBean.setNomeTaskFase(stringaSplittata[i] + ";" + stringaSplittata[i+1] );
		...
		//Invio alla verifica contabile - SVOLTO
		...
		//Rilasciata approvazione contabile - SVOLTO
		...
		if (assegnatario != null){
			...
			for(int i = 0; i < assegnatario.size(); i++) {
				...
				assegnatarioXmlBean.setRuolo("ASSEGNATARIO_BILANCIO");
		...
		for (AttiXmlBean attoXmlBean : storedProcedureResults){			
			...
			currentRetrievedBean.setIdModDispositivo(null);
			currentRetrievedBean.setNomeModDispositivo(null);
			currentRetrievedBean.setDisplayFilenameModDispositivo(null);
			returnValue.add(currentRetrievedBean);
		}
		
		return new PaginatorBean<AttiBean>(returnValue);
	}

...
public class SelezionaTipoAttoForm extends DynamicForm {
	...
	public SelezionaTipoAttoForm(SelezionaTipoAttoWindow pWindow){
		...
		idTipoProc.setOptionDataSource(new GWTRestDataSource("LoadComboAttoConFlussoWFDataSource"));		
		...
		setFields(idTipoProc, nomeTipoProc, idTipoFlussoActiviti, idTipoDocProposta, nomeTipoDocProposta, siglaProposta, okButton);
				
	}
	
	protected void manageOnClick() {
		
		final Record lFormRecord = getValuesAsRecord();
		
		window.markForDestroy();
		
		final ModalWindow avvioIterModalWindow = new ModalWindow("avvio_iter_atti", true, false) {
			@Override
			public void manageOnCloseClick() {
				markForDestroy();
			}
		};
		avvioIterModalWindow.setTitle("Avvio proposta di " + lFormRecord.getAttribute("nomeTipoProc").toLowerCase());
		avvioIterModalWindow.setIcon("menu/iter_atti.png");
		avvioIterModalWindow.setMaximized(true);
		avvioIterModalWindow.setIsModal(false); //visto che è massimizzata la rendo non modale per far funzionare il tabulatore (Mattia Zanin)
		...
		lVLayout.setTop(5);
		AvvioPraticaLayout avvioPraticaLayout = new AvvioPraticaLayout(lFormRecord, avvioIterModalWindow);
	...
	public void avvioIterAtti(Record lFormRecord, String idUd, String idFolder, String uoProtocollante, String oggetto, final ModalWindow avvioIterAttiWindow) {
		final String idTipoProc = lFormRecord.getAttribute("idTipoProc");
		final String nomeTipoProc = lFormRecord.getAttribute("nomeTipoProc");
		final String idTipoFlussoActiviti = lFormRecord.getAttribute("idTipoFlussoActiviti");
		final String idTipoDocProposta = lFormRecord.getAttribute("idTipoDocProposta");
		final String nomeTipoDocProposta = lFormRecord.getAttribute("nomeTipoDocProposta");
		final String siglaProposta = lFormRecord.getAttribute("siglaProposta");
					...
					public void execute(DSResponse response, Object rawData, DSRequest request) {															
						if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
							...
							for(int i = 0; i < listaAttivita.getLength(); i++) {
								...
								boolean isEseguibile = lRecordTask.getAttribute("flgEseguibile") != null && "1".equals(lRecordTask.getAttribute("flgEseguibile"));									
								...
								if(isEseguibile) {
									...
									lGWTRestService.call(lRecord, new ServiceCallback<Record>() {
										...
										public void execute(Record object) {
											...
											GWTRestService<Object, Object> lAurigaTaskDataSource = new GWTRestService<Object, Object>("AurigaTaskDataSource");
											lAurigaTaskDataSource.executecustom("salvaTask", lRecord, new DSCallback() {
												@Override
												public void execute(DSResponse response, Object rawData, DSRequest request) {
													if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
														avvioIterAttiWindow.markForDestroy();	

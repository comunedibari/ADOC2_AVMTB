 ...
 *
 */
public class AttiPersonaliLayout extends CustomLayout {

	protected MultiToolStripButton smistaMultiButton;
	...
	public AttiPersonaliLayout(String nomeEntita, String finalita, Boolean flgSelezioneSingola, Boolean showOnlyDetail) {
		super(nomeEntita, new GWTRestDataSource("AttiPersonaliDatasource"), createFilter(nomeEntita), new AttiPersonaliList(nomeEntita), new AttiDetail(
				nomeEntita), finalita, flgSelezioneSingola, showOnlyDetail);

		if (isAttivoSmistamentoAtti()) {
			setMultiselect(true);
		} else {
	...
	public static boolean isAttivoCMMI() {
	...
	protected MultiToolStripButton[] getMultiselectButtons() {
		...
		if (isAttivoSmistamentoAtti()) {
			if (smistaMultiButton == null) {
				smistaMultiButton = new MultiToolStripButton("archivio/assegna.png", this, "Smista", false) {
					...
					public void doSomething() {
						...
						final SmistamentoAttiPopup smistamentoAttiPopup = new SmistamentoAttiPopup((listaRecord.getLength() == 1) ? listaRecord.get(0) : null) {
							...
							public void onClickOkButton(final DSCallback callback) {
								...
								Layout.showWaitPopup("Smistamento atti in corso: potrebbe richiedere qualche secondo. Attendere…");
						...
						smistamentoAttiPopup.show();
					}
				};
			}
			return new MultiToolStripButton[] { smistaMultiButton };
		} else

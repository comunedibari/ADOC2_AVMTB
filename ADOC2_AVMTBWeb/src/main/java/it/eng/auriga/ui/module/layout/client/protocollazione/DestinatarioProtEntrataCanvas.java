...
public class DestinatarioProtEntrataCanvas extends IndirizzoCanvas {
	...
	public void buildMainForm() {
		...
		styleMap.put("LD", I18NUtil.getMessages().protocollazione_select_listmap_LD_value());
//		styleMap.put("PREF", I18NUtil.getMessages().protocollazione_select_listmap_PREF_value());
		tipoDestinatarioItem.setAllowEmptyValue(true);
//		tipoDestinatarioItem.setRequired(true);
		tipoDestinatarioItem.setShowTitle(false);
		tipoDestinatarioItem.setValueMap(styleMap);
...
//		opzioniInvioAssegnazioneButton.setAlwaysEnabled(true);
		...
		opzioniInvioAssegnazioneButton.setShowIfCondition(new FormItemIfFunction() {
			...
			@Override
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				return showFlgAssegnaAlDestinatario && flgAssegnaAlDestinatarioItem.getValueAsBoolean() /*&& !((DestinatarioProtEntrataItem) getItem()).getShowAggiungiDestInAssegnatariButton()*/;
			}
		});
...
//		});
		...
		mDynamicForm.setFields(
			...
			cercaInRubricaButton, cercaInOrganigrammaButton, salvaInRubricaButton, flgAssegnaAlDestinatarioItem, 
			opzioniInvioHiddenItem, opzioniInvioAssegnazioneButton, /*aggiungiDestInAssegnatariButton,*/ destinatariPreferitiItem,showMembriGruppoButton
		);
	...
	public void showHideFlgAssegnaAlDestinatario() {		
		...
		if (((DestinatarioProtItem) getItem()).getShowFlgAssegnaAlDestinatario() &&  isValorizzatoSoggPerAssegnazione()) {
			if (!fromLoadDett /*&& !((DestinatarioProtEntrataItem) getItem()).getShowAggiungiDestInAssegnatariButton()*/ && mDynamicForm.getValue("flgAssegnaAlDestinatario") == null) {				
				boolean flgSelXAssegnazione = mDynamicForm.getValueAsString("flgSelXAssegnazione") != null && "1".equals(mDynamicForm.getValueAsString("flgSelXAssegnazione"));				

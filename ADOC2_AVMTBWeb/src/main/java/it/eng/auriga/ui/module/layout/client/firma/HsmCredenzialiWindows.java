...
//	private boolean firmaHsmRequireCaricamentoCertificati;
	...
	public HsmCredenzialiWindows(String providerHsm, Record preimpostazioni) {
		
		this.providerHsm = providerHsm;
...
//		this.firmaHsmRequireCaricamentoCertificati = firmaHsmRequireCaricamentoCertificati;
		...
		if(usernameDelegante != null && !"".equals(usernameDelegante) && !nascondiUsernameDelegante()) {
	...
	protected HLayout createUsernameLayout() {
		...
		deleganteTextItem.setShowIfCondition(new FormItemIfFunction() {
			...
			@Override
			public boolean execute(FormItem arg0, Object arg1, DynamicForm arg2) {
				return !nascondiUsernameDelegante();
			}
		});
	...
	protected HLayout createOtpLayout() {
		...
		generazioneRemotaOtpSmsImgButton.setCanEdit(false);
					...
					if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
						...
						if (data.getAttributeAsBoolean("esitoOk")) {										
							...
							if ((listaRecordCertificati != null) && (listaRecordCertificati.length > 0)){
								...
								passwordTextItem.setTextBoxStyle(it.eng.utility.Styles.textItem);
								otpCodeTextItem.setCanEdit(true);
								generazioneRemotaOtpSmsImgButton.setCanEdit(true);
							}else{
	...
	public abstract boolean nascondiUsernameDelegante();

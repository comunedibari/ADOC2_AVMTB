...
public class PreferenceDataSource extends AbstractFetchDataSource<PreferenceBean>{	
	...
	public PaginatorBean<PreferenceBean> fetch(AdvancedCriteria criteria,Integer startRow, Integer endRow, List<OrderByBean> orderByBeanList)throws Exception {
		...
		Collections.sort(pagingList.getData(), new Comparator<PreferenceBean>() {
			...
			public int compare(PreferenceBean pref1, PreferenceBean pref2) {
				...
				return pref1.getPrefName().compareTo(pref2.getPrefName());
			}
		});
		
		paginatorBean.setData(pagingList.getData());
...
//		}
		...
		paginatorBean.setStartRow(pagingList.getStartRow());
		paginatorBean.setEndRow(pagingList.getEndRow());
		paginatorBean.setTotalRows(pagingList.getTotalRows());
	...
	public PreferenceBean get(PreferenceBean bean) throws Exception {
		...
		if(StringUtils.isNotBlank(bean.getUserid()) && StringUtils.isNotBlank(bean.getPrefName()) && StringUtils.isNotBlank(bean.getPrefKey())) {
			return AurigaService.getDaoTUserPreferences().get(getLocale(), AurigaUserUtil.getLoginInfo(getSession()), bean);	
		} else {
	...
	@Override
	public PreferenceBean update(PreferenceBean bean, PreferenceBean oldvalue)
	throws Exception {
		if(StringUtils.isBlank(bean.getUserid())) {
			String userId = getExtraparams().get("userId");
			if(StringUtils.isBlank(userId)) {

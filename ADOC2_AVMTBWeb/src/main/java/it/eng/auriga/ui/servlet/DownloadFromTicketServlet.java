...
public class DownloadFromTicketServlet {
	...
	public HttpEntity<byte[]> download(HttpServletRequest req, HttpServletResponse resp) throws Exception {
		...
		String fltTpDominioInput = req.getParameter("tpDominio");

		String idDominioInput = req.getParameter("idDominio");
		...
		if (ticketId == null || ticketId.equals("")) {
			error(resp, "Parametro TicketId non presente sulla request");
		}

		else if (schema == null || schema.equals("")) {
			error(resp, "Parametro schema non presente sulla request");
		}

		/**
		 * Controllo di regressione per DigiDoc, in quanto ha la presenza di mail con un solo link
		 */
		else if ((fltTpDominioInput == null || fltTpDominioInput.equals("")) || (idDominioInput == null || idDominioInput.equals(""))) {

			loginBean.setToken("#RESERVED_FATTAIRLIQUIDE_673");
			try {
				resultDownloadTicket = AurigaService.getDaoDownloadTicket().search(new Locale("it"), loginBean, filter);
				if (resultDownloadTicket.getData() != null && resultDownloadTicket.getData().size() > 0) {
						...
						if (idUdFattura != null) {
							...
							try {
								...
								if (StringUtils.isNotBlank(lUpddocudOutput.getDefaultMessage())) {
									String message = "Non riesco a fare update = " + ticketId + " - DMPK_CORE.UpdDocUd: " + lUpddocudOutput.getDefaultMessage();
								...
								error(resp, e3.getMessage());
							}
						} else {
							error(resp, "ID UD non trovato. Impossibile scaricare il file.");
						}
					} else {
						error(resp, "URI file non trovato. Impossibile scaricare il file.");
					}
				} else {
					error(resp, "Nessun file trovato con ticketId = " + ticketId);
				}
			} catch (Exception e) {
				String mess = "Impossibile aprire il file con ticket = " + ticketId + ". ";
				if (e.getMessage() != null && !e.getMessage().equalsIgnoreCase(""))
					mess = mess + "Errore = " + e.getMessage();
				error(resp, mess);
			}
		} else {

			setParametriLoginFromDVB(fltTpDominioInput, idDominioInput, lSchemaBean);
				...
				if (resultDownloadTicket.getData() != null && resultDownloadTicket.getData().size() > 0) {
					...
					if (uri != null && !uri.equals("")) {
						if (idUdFattura != null) {
							...
							boolean flgAttivaAggStatoUdDownloadLink = ATTIVA_AGG_STATO_UD_DOWNLOD_LINK != null && !"".equals(ATTIVA_AGG_STATO_UD_DOWNLOD_LINK)
									&& ("1".equals(ATTIVA_AGG_STATO_UD_DOWNLOD_LINK) || "true".equals(ATTIVA_AGG_STATO_UD_DOWNLOD_LINK)) ? true : false;
							if (flgAttivaAggStatoUdDownloadLink) {
								...
								try {
									...
									if (isVersioneNESTLE()){
										attributiUdFattura.setCodStato("2");	
										attributiUdFattura.setCodStatoDett("CONS");
									}
									else{
										attributiUdFattura.setCodStato("CONS");	
									}
									...
									if (StringUtils.isNotBlank(lUpddocudOutput.getDefaultMessage())) {
										String message = "Non riesco a fare update = " + ticketId + " - DMPK_CORE.UpdDocUd: "
												+ lUpddocudOutput.getDefaultMessage();
										if (lUpddocudOutput.isInError()) {
											error(resp, message);
											return null;
										}
	...
	private void setParametriLoginFromDVB(String fltTpDominioInput, String idDominioInput, SchemaBean lSchemaBean) throws Exception, StoreException {
		...
		if (StringUtils.isNotBlank(resultLogin.getDefaultMessage())) {
			if (resultLogin.isInError()) {
				throw new StoreException(resultLogin.getDefaultMessage());
			}
		}
	...
	public boolean isVersioneNESTLE() {     
        boolean ret = false;        
         if (SPEC_LABEL_GUI !=null){
               if (SPEC_LABEL_GUI.equalsIgnoreCase("NESTLE")){
                  ret = true;
                }
        }
       return ret;

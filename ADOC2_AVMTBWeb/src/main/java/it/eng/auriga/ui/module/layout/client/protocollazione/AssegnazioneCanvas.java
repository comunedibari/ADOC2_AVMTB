...
//		mDynamicForm.setCellBorder(1);
		...
		if(((AssegnazioneItem) getItem()).isProtInModalitaWizard() && AurigaLayout.isAttivaFinalitaForRestrAssCartaceo()) {			
			organigrammaDS.addParam("finalita", ((AssegnazioneItem) getItem()).isAttivaRestrAssCartaceoProt() ? "ASS_CARTACEO" : "ASS_NO_CARTACEO");
		} else {
			organigrammaDS.addParam("finalita", "ASS");
		}
...
//		opzioniInvioAssegnazioneButton.setAlwaysEnabled(true);
	...
	public class AssegnazioneLookupOrganigramma extends LookupOrganigrammaPopup {
		...
		public String getFinalita() {
			if(((AssegnazioneItem) getItem()).isProtInModalitaWizard() && AurigaLayout.isAttivaFinalitaForRestrAssCartaceo()) {				
				return ((AssegnazioneItem) getItem()).isAttivaRestrAssCartaceoProt() ? "ASS_CARTACEO" : "ASS_NO_CARTACEO";
			} else
				return "ASSEGNAZIONE";
		}
	...
	public LinkedHashMap<String, String> buildTipoValueMap() {
		...
		if(!((AssegnazioneItem) getItem()).isAttivoAssegnatarioUnicoCartaceoProt() && !flgSenzaLD) { 

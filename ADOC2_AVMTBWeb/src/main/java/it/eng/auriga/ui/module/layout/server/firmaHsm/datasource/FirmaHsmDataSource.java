...
import it.eng.hsm.cades.HsmCadesUtility;
...
import it.eng.hsm.pades.HsmPadesUtility;
...
public class FirmaHsmDataSource extends AbstractDataSource<FirmaHsmBean, FirmaHsmBean> {
	...
	public FirmaHsmBean richiediCodiceOtp(FirmaHsmBean bean) {
		...
		// Ricavo il provider con cui effettuare la firma HSM
		String providerHsm = ParametriDBUtil.getParametroDB(getSession(), "TIPO_HSM");
		bean.setHsmType(providerHsm);
		...
		try {
			...
			log.error("Errore nella richiesta del codice OTP", e);
	...
	public FirmaHsmBean richiediListaCertificati(FirmaHsmBean bean) {
		...
		try {
			...
			log.error("Errore nella richiesta della lista dei certificati", e);
			bean.setEsitoOk(false);
			String messaggioErrore;
			if (StringUtils.isNotBlank(e.getMessage())) {
				messaggioErrore = e.getMessage();
	...
	public FirmaHsmBean firmaHsmHashMultipla(FirmaHsmBean bean) {
		...
		// Dirotto la firma analizzando il tipo di firma impostata, l'estensione del file e se il file è prefirmato
			...
			if ((hsmTipoFirma != null) && (hsmTipoFirma.equalsIgnoreCase("PAdES"))) {
				...
				if (fileDaFirmare.getNomeFile().toLowerCase().endsWith(".pdf")) {
					...
					if ((fileDaFirmare.getInfoFile() != null) && ((!fileDaFirmare.getInfoFile().isFirmato())
							|| (fileDaFirmare.getInfoFile().isFirmato()) && (fileDaFirmare.getInfoFile().isFirmaValida()))) {
					...
					if (firmaCongiunta) {
						...
						if ((fileDaFirmare.getInfoFile() != null) && (fileDaFirmare.getInfoFile().isFirmato())
								&& (fileDaFirmare.getInfoFile().isFirmaValida())) {
				...
				if (firmaCongiunta) {
					...
					// devono essere firmati in modo verticale
					log.debug("L'utilizzo della firma congiunta è impostato a true");
					if ((fileDaFirmare.getInfoFile() != null) && (fileDaFirmare.getInfoFile().isFirmato()) && (fileDaFirmare.getInfoFile().isFirmaValida()) && (fileDaFirmare.getInfoFile().getTipoFirma().startsWith("CAdES"))) {
						log.debug("Il file è firmato e valido, uso la CAdES congiunta");
						fileDaFirmare.setTipoFirma(TipoFirmaHsm.HASH_CADES_CONGIUNTA);
					} else {

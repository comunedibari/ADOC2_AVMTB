...
//					lRecord.setAttribute("flgAssegnaAlDestinatario", false); // non posso settare il check a false altrimenti non viene caricato successivamente  
		...
		codfiscaleDestinatarioItem.setWidth(120);
		...
		ImgButtonItem salvaInRubricaButton = new ImgButtonItem("salvaInRubricaButton", "buttons/saveIn.png",
		...
		salvaInRubricaButton.setShowIfCondition(new FormItemIfFunction() {
			...
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				...
				String tipoDestinatario = tipoDestinatarioItem.getValueAsString();
				return Layout.isPrivilegioAttivo("GRS/S/UO;I") && (tipoDestinatario == null || "".equals(tipoDestinatario)
						|| ((isPersonaFisica(tipoDestinatario) || isPersonaGiuridica(tipoDestinatario)) && !"AOOI".equalsIgnoreCase(tipoDestinatario)
								&& !"PA".equalsIgnoreCase(tipoDestinatario)));
			}
		});
		...
		// check p.c.
		flgPCItem = new CheckboxItem("flgPC", I18NUtil.getMessages().protocollazione_detail_flgPCItem_title());
		...
		flgPCItem.addChangedHandler(new ChangedHandler() {
			...
			@Override
			public void onChanged(ChangedEvent event) {
				flgAssegnaAlDestinatarioItem.setValue(false);
			}
		});
		flgPCItem.setShowIfCondition(new FormItemIfFunction() {
			...
			@Override
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				return AurigaLayout.getParametroDBAsBoolean("SHOW_CC_IN_DEST_REG");
			}
		});

		// check assegna
		flgAssegnaAlDestinatarioItem = new CheckboxItem("flgAssegnaAlDestinatario", I18NUtil.getMessages().protocollazione_detail_flgAssegnaItem_title());
				...
				if(idAssegnatario != null && !"".equals(idAssegnatario)) {
					...
					if((idAssegnatario.startsWith("UO") && idUoSoggetto.equals(idAssegnatario.substring(2))) ||
					   ...
					   (idAssegnatario.startsWith("UT") && idUserSoggetto.equals(idAssegnatario.substring(2)))) {
						...
						item.setTitle("effettuata assegnazione");
...
//		opzioniInvioAssegnazioneButton.setAlwaysEnabled(true);
		...
		opzioniInvioAssegnazioneButton.setShowIfCondition(new FormItemIfFunction() {
			...
			@Override
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				return showFlgAssegnaAlDestinatario && flgAssegnaAlDestinatarioItem.getValueAsBoolean();
			}
		});
		...
		mDynamicForm.setFields(
			idDestinatarioItem, idSoggettoItem, idUoSoggettoItem, idUserSoggettoItem, idScrivaniaSoggettoItem, idAssegnatarioItem, flgSelXAssegnazioneItem, 
			gruppoSalvatoItem,tipoDestinatarioItem,
			iconaFlgInOrganigrammaItem, codRapidoDestinatarioItem, lookupRubricaButton, lookupOrganigrammaButton, flgPCItem, denominazioneDestinatarioItem,
			cognomeDestinatarioItem, nomeDestinatarioItem, codfiscaleDestinatarioItem, gruppiDestinatarioItem, idGruppoInternoItem, idGruppoEsternoItem,
			descrAoomdgDestinatarioItem, aoomdgDestinatarioItem, cercaInRubricaButton, cercaInOrganigrammaButton, salvaInRubricaButton,	flgAssegnaAlDestinatarioItem, 
			opzioniInvioHiddenItem, opzioniInvioAssegnazioneButton, mezzoTrasmissioneDestinatarioItem, destinatariPreferitiItem,showMembriGruppoButton
		);
...
//			return AurigaLayout.getParametroDBAsBoolean("SHOW_AOOI_IN_MITT_DEST") ? new String[] { "AOOI", "UOI", "UP", "PA", "PF", "PG"} : new String[] {"UOI", "UP", "PA", "PF", "PG"};
	...
	protected void cercaSoggetto(Record lRecord, ServiceCallback<Record> callback) {
		GWTRestService<Object, Object> lGwtRestService = new GWTRestService<Object, Object>("FindSoggettoDatasource");		
//		lGwtRestService.addParam("tipiAmmessi", AurigaLayout.getParametroDBAsBoolean("SHOW_AOOI_IN_MITT_DEST") ? "AOOI,UOI,UP,PA,PF,PG" : "UOI,UP,PA,PF,PG");			
	...
	public boolean isTipoSoggettoInterno() {
		if(tipoDestinatarioItem.getValueAsString() != null && (tipoDestinatarioItem.getValueAsString().equals("UOI") || tipoDestinatarioItem.getValueAsString().equals("UP"))) {						
			return true;
		}
		return false;
	}

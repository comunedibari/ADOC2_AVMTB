...
class AlboDetail extends Component {
	...
	async before_render() {
		...
		if (this.idUD) {
			...
			await AlboService.get(this.idUD)
			.then(res => {
				that.detail = res;
			})
			.catch((error) => {
	...
	async render() {
		...
		detailHtml += `<strong class="text-secondary">Origine&nbsp;</strong>${this.detail.richiedente}<br>`;
		detailHtml += `<strong class="text-secondary">Tipologia&nbsp;</strong>${this.detail.tipo}<br>`;
		detailHtml += `<strong class="text-secondary">Stato pubblicazione&nbsp;</strong>${this.detail.stato}</span>`;
		detailHtml += '</div>';
		...
		if (this.detail.stato && this.detail.stato.toUpperCase() === "ANNULLATA") {
			...
			<div class="row">
			<strong class="text-secondary">Testo atto&nbsp;</strong> ${new File(this.detail.filePrimario)} ${new IconButton("Scarica tutti i file come zip", "downloadZip", downloadzipfun, "file-archive")}

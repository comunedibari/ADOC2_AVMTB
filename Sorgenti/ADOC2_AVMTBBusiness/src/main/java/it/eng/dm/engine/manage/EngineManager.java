...
public class EngineManager {
	...
	private void init() {		
		if(mapProcessEngine == null) {
			...
			if(lActivitiSessionFileConfigurator != null) {				
				for (SessionFile lSessionFile : lActivitiSessionFileConfigurator.getSessions()){
					if(mapProcessEngine == null) mapProcessEngine = new HashMap<String, ProcessEngine>();
					PerformanceLogger lPerformanceLogger = new PerformanceLogger(getUserInfoForPerformanceLogger(), "ProcessEngineConfiguration.createProcessEngineConfigurationFromResource(\"" + lSessionFile.getFileName() + "\").buildProcessEngine()");
				...
				PerformanceLogger lPerformanceLogger = new PerformanceLogger(getUserInfoForPerformanceLogger(), "ProcessEngineConfiguration.createProcessEngineConfigurationFromResource(\"activiti.cfg.xml\").buildProcessEngine()");
				lPerformanceLogger.start();

...
public class DettaglioPostaElettronica extends CustomDetail {
	...
	protected DettaglioPostaElettronicaToolStripButton protocollaRegistraRepertorioToolStripButton;
...
//				RecordList listaDestinatariPreferiti = destinatariPreferiti.getAttributeAsRecordList("listaUtentiPreferitiMail");
...
//				RecordList listaDestinatariPreferiti = destinatariPreferiti.getAttributeAsRecordList("listaUtentiPreferitiMail");
		...
		// Protocolla / registra email
		protocollaRegistraRepertorioToolStripButton = new DettaglioPostaElettronicaToolStripButton(I18NUtil.getMessages().posta_elettronica_list_registraMenuItem(),
				"buttons/protocollazione.png");
		protocollaRegistraRepertorioToolStripButton.addClickHandler(new com.smartgwt.client.widgets.events.ClickHandler() {

			@Override
			public void onClick(com.smartgwt.client.widgets.events.ClickEvent event) {
				
				//se è abilitato solo alla protocollazione, protocolla mail
				if (isAttivaAbilitazione("abilitaProtocolla") && !isAttivaAbilitazione("abilitaRepertoria")) {
					manageRegistraProtocollaCick();
					
				//se è abilitato alla registrazione del repertorio, registra a repertorio
				} else if (!isAttivaAbilitazione("abilitaProtocolla") && isAttivaAbilitazione("abilitaRepertoria")) {
					registraRepertorioClick();
				}
...
//							final RecordList listaDestinatariPreferiti = destinatariPreferiti.getAttributeAsRecordList("listaUOPreferiteMail");
			...
			private Menu buildScelteRapideAssegnaUO(Record destinatariPreferiti){
				...
				Menu scelteRapide = new Menu();
				Boolean success = destinatariPreferiti.getAttributeAsBoolean("success");
//				RecordList listaDestinatariPreferiti = destinatariPreferiti.getAttributeAsRecordList("listaUOPreferiteMail");
				...
				Scheduler.get().scheduleDeferred(new ScheduledCommand() {
					...
					public void execute() {
						if ((portletLayout != null) && (portletLayout.getItemLavorazioneMailItem() != null)
							...
							GWTRestService<Object, Object> lGwtRestService = new GWTRestService<Object, Object>("AurigaSalvaIterBozzaMailDataSource");
	...
	private void onClickInoltraCommmon(final Record listRecordInoltra , final boolean allegaMailOrig) {
		...
		final GWTRestService<Object, Object> lGwtRestServiceInvio = new GWTRestService<Object, Object>("AurigaInvioMailDatasource");
	...
	private void populateDetailButtons(List<DetailToolStripButton> detailButtons, Record record) {
		if (isBozza(record)) {
			...
			/*
			 * In arrivo interoperabile In arrivo (se non interoperabile) e solo se la mail non è protocollata (o almeno non tutti i suoi gli allegati)
			 */
			detailButtons.add(protocollaRegistraRepertorioToolStripButton);
	...
	private void buildFormEstremi() {
		...
		iconaMicroCategoriaItem.setShowIfCondition(new FormItemIfFunction() {
			...
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				...
				// FIXME Questo hint causa problemi nel layout
		...
		titleItemProtocollataCome.setShowIfCondition(new FormItemIfFunction() {
			...
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				...
				if (statoProtocollazione != null && "protocollata".equals(statoProtocollazione)) {
					...
					titleItemProtocollataCome.setDefaultValue("Registrata come :");
				}
				return isMailEntrataI() && isMailProtocollata();
			}
		});
...
//		titleItemMittenteEmailInviata.setDefaultValue("Da :");
...
//		titleItemIndirizzoDestinatariOriginali.setDefaultValue("A :");
...
//		titleItemIndirizzoDestinatariCopia.setDefaultValue("CC :");
...
//	}
	...
	protected void buildFormContenuti() {
		...
		lCustomValidator.setErrorMessage(I18NUtil.getMessages().invionotificainteropform_destinatariValidatorErrorMessage());
		lTextItemDestinatariBozza.setValidators(lCustomValidator);
		...
		destinatariCCNAttoriItem.setTitle("CCN");
		...
		titleItemOggettoDatiProtocollo.setDefaultValue("Oggetto :");
		...
		titleItemCorpoMail.setDefaultValue("Corpo :");
		...
		mDynamicFormContenuti.setFields(titleItemMittenteEmailInviata, mittenteEmailInviataAttoriItem, lSelectItemMittenteBozza,
				...
				destinatariPrincipaliAttoriItem, destinatariCopiaAttoriItem, destinatariCCNAttoriItem, 
	...
	protected void buildFormProtocolloMitt() {
		...
		titleEnteProtocollo.setDefaultValue("Da :");
		...
		titleOggettoMailProtocollo.setDefaultValue("Oggetto :");
	...
	public void editRecord(Record record, boolean fromLivello) {
		...
		if(isAttivaAbilitazione("abilitaRepertoria", record) && isAttivaAbilitazione("abilitaProtocolla", record)) {
				...
				protocollaRegistraRepertorioToolStripButton.hide();
		}
		//Se è abilitato o alla protocollazione o alla registrazione  del repertorio mostra solo il tasto registra
		//che protocollerà o registrerà a repertorio a seconda dell abilitazione
		else if(isAttivaAbilitazione("abilitaRepertoria", record) || isAttivaAbilitazione("abilitaProtocolla", record)){
				registraToolStripButton.hide();
				protocollaRegistraRepertorioToolStripButton.show();
		}
		...
		if (isAttivaAbilitazione("abilitaNotifInteropEccezione", record) && openDetailSectionProtocollo()
									...
									sbloccaEmail(pRecordMail, new ServiceCallback<Record>() {
										...
										public void execute(Record data) {
											...
											if (errorMessages != null && errorMessages.size() > 0) {
												...
												final GWTRestService<Object, Object> lGwtRestServiceInvioNotifica = new GWTRestService<Object, Object>(
	...
	protected void managerInvioNotificaInteropResponse(final GWTRestService<Object, Object> lGwtRestServiceInvioNotifica, final Record pRecordMail) {
											...
											if (errorMessages != null && errorMessages.size() > 0) {
												...
												lock(listaEmail, new ServiceCallback<Record>() {
													...
													public void execute(Record data) {
														...
														GWTRestService<Object, Object> lGwtRestServiceAurigaInvioMailDatasource = new GWTRestService<Object, Object>(
													...
													lock(listaEmail, new ServiceCallback<Record>() {
														...
														public void execute(Record data) {
															...
															if (isNotObbligatoria(pRecordMail)) {
																GWTRestService<Object, Object> lGwtRestService = new GWTRestService<Object, Object>(
																		"AurigaInvioMailDatasource");
																lGwtRestService.extraparam.put("tipoRel", tipoRel);
						...
						lock(listaEmail, new ServiceCallback<Record>() {
							...
							public void execute(Record data) {
								...
								if (isNotObbligatoria(pRecordMail)) {
									...
									GWTRestService<Object, Object> lGwtRestService = new GWTRestService<Object, Object>("AurigaInvioMailDatasource");
									lGwtRestService.extraparam.put("tipoRel", tipoRel);
									lGwtRestService.executecustom("invioBozzaMail", pRecordMail, new DSCallback() {
	...
	protected void manageRegistraIstanzaClick(Record pRecord, final String nomeEntita) {
		GWTRestService<Object, Object> lGwtRestServiceRegistraIstanza = new GWTRestService<Object, Object>("AurigaProtocollaPostaElettronicaDataSource");
	...
	private void manageStampaTuttiIFile() {
		...
		if (sizeListaAllegati == sizeListaFileNoteAppunti) {
			...
			// Ci sono file da stampare e non
				...
				SC.ask("Alcuni dei file da stampare: " + value + " hanno formato non stampabile", new BooleanCallback() {
	...
	private void manageStampaAllegati() {
		...
		if (recordListAllegati != null && recordListAllegati.getLength() > 0 && !recordListAllegati.isEmpty()) {
			...
			// Ci sono sia file stampabili che non
				...
				SC.ask("Alcuni degli allegati da stampare: " + value + " hanno formato non stampabile, Procedi ?", new BooleanCallback() {
	...
	private void manageStampaFileAppuntiNote() {
		...
		if (recordListAppuntiNote != null && recordListAppuntiNote.getLength() > 0 && !recordListAppuntiNote.isEmpty()) {
			...
			// Ci sono sia file stampabili che non
				...
				SC.ask("Alcuni degli file da stampare: " + value + " hanno formato non stampabile, Procedi ?", new BooleanCallback() {
...
//		firmaPredefinitaButton.setBackgroundImage("");
	...
	private void manageRegistraProtocollaCick() {
		ricaricaPagina = true;
		final Record listRecordProtocolla = new Record();
		listRecordProtocolla.setAttribute("idEmail", new Record(vm.getValues()).getAttribute("idEmail"));
		listRecordProtocolla.setAttribute("flgIo", new Record(vm.getValues()).getAttribute("flgIO"));
		...
		if(!AurigaLayout.getImpostazioniDocumentoAsBoolean("skipSceltaTipologiaEntrata")) {
			...
			AurigaLayout.apriSceltaTipoDocPopup(false, idTipoDocDefault, "PG", null, new ServiceCallback<Record>() {
				...
				public void execute(Record lRecordTipoDoc) {
					...
					manageProtocollaClick(listRecordProtocolla, destinatariEAccount, idTipoDoc, nomeTipoDoc);
				}
			});
		} else {
	...
	protected void manageProtocollaClick(final Record pRecord, final String destinatariEAccount, final String idTipoDoc, final String nomeTipoDoc) {
		...
		if(!filedaAppunti.isEmpty()) {
			...
			SC.ask("Sono stati trovati dei file nella sezione Appunti & Note, vuoi aggiungerli come allegati alla protocollazione?", new BooleanCallback() {					
				@Override
				public void execute(Boolean value) {
					
					if(value) {
						GWTRestService<Object, Object> lGwtRestServiceProtocolla = new GWTRestService<Object, Object>("AurigaProtocollaPostaElettronicaDataSource");
						pRecord.setAttribute("filedaAppunti",filedaAppunti);
						lGwtRestServiceProtocolla.extraparam.put("idTipoDoc", idTipoDoc);
						lGwtRestServiceProtocolla.extraparam.put("nomeTipoDoc", nomeTipoDoc);
						lGwtRestServiceProtocolla.call(pRecord, new ServiceCallback<Record>() {

							@Override
							public void execute(Record object) {
								manageProtocollaPostaElettronicaCallback(object, destinatariEAccount);
							}
						});
					} else {
						GWTRestService<Object, Object> lGwtRestServiceProtocolla = new GWTRestService<Object, Object>("AurigaProtocollaPostaElettronicaDataSource");
	...
	protected void manageProtocollaPostaElettronicaCallback(Record lRecord, String destinatariEAccount) {
	...
	private void registraRepertorioClick() {
		
		ricaricaPagina = true;
		final Record recordRepertoria = new Record();
		recordRepertoria.setAttribute("idEmail", new Record(vm.getValues()).getAttribute("idEmail"));
		recordRepertoria.setAttribute("flgIo", new Record(vm.getValues()).getAttribute("flgIO"));
		final String destinatariEAccount = new Record(vm.getValues()).getAttributeAsString("casellaAccount");
		...
		if (AurigaLayout.getImpostazioniDocumentoAsBoolean("skipSceltaRepertorioEntrata")) {
			if (AurigaLayout.getImpostazioniDocumentoAsBoolean("skipSceltaTipologiaRepertorioEntrata")) {
				...
				manageRegistraRepertorio(recordRepertoria,destinatariEAccount,idTipoDoc,nomeTipoDoc, repertorioEntrata);
			} else {
				String idTipoDocDefault = AurigaLayout.getImpostazioniDocumento("idTipoDocumentoRepertorioEntrata");
				AurigaLayout.apriSceltaTipoDocPopup(true, idTipoDocDefault, "R", repertorioEntrata, new ServiceCallback<Record>() {
			...
			AurigaLayout.apriSceltaRepertorioPopup("E", repertorioEntrata, new ServiceCallback<Record>() {

				@Override
				public void execute(Record lRecordRepertorio) {

					final String repertorio = lRecordRepertorio != null ? lRecordRepertorio.getAttribute("repertorio") : null;
					if (repertorio != null && !"".equals(repertorio)) {
						AurigaLayout.apriSceltaTipoDocPopup(true, null, "R", repertorio, new ServiceCallback<Record>() {

							@Override
							public void execute(Record lRecordTipoDoc) {
								String idTipoDoc = lRecordTipoDoc != null ? lRecordTipoDoc.getAttribute("idTipoDocumento") : null;
								String nomeTipoDoc = lRecordTipoDoc != null ? lRecordTipoDoc.getAttribute("descTipoDocumento") : null;
								manageRegistraRepertorio(recordRepertoria,destinatariEAccount,idTipoDoc,nomeTipoDoc, repertorio);
							}
						});
					}
				}
	...
	private void manageRegistraRepertorio(final Record lRecord, final String destinatariEAccount, 
					final String idTipoDoc, final String nomeTipoDoc, final String repertorio){
		GWTRestDataSource lGwtRestDataSource = new GWTRestDataSource("AurigaGetDettaglioPostaElettronicaDataSource", "idEmail", FieldType.TEXT);
		lGwtRestDataSource.performCustomOperation("get", lRecord, new DSCallback() {
			...
			public void execute(DSResponse response, Object rawData, DSRequest request) {
				if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
					...
					if(!filedaAppunti.isEmpty()) {
						// A video popup richiesta utente se vuole aggiungere o meno i file. Inserire la lista come extraparam.
						SC.ask("Sono stati trovati dei file nella sezione Appunti & Note, vuoi aggiungerli come allegati alla repertoriazione?", new BooleanCallback() {					
							@Override
							public void execute(Boolean value) {
								...
								if(value) {
									...
									lGwtRestServiceProtocolla.call(lRecord, new ServiceCallback<Record>() {
										...
										@Override
										public void execute(Record object) {
											manageRegistraRepertorioCallback(object, destinatariEAccount, idTipoDoc, nomeTipoDoc, repertorio);
										}
									});
								} else {
									GWTRestService<Object, Object> lGwtRestServiceProtocolla = new GWTRestService<Object, Object>("AurigaProtocollaPostaElettronicaDataSource");
	...
	private void manageRegistraRepertorioCallback(Record lRecord, String destinatariEAccount, String idTipoDoc, String nomeTipoDoc,String repertorio){
		if (destinatariEAccount != null) {
			lRecord.setAttribute("casellaEmailDestinatario", destinatariEAccount); // viene settato il campo casellaEmailDestinatario del bean
		}
		...
		RegistraRepertorioEmailWindow lRegistraRepertorioEmailWindow = new RegistraRepertorioEmailWindow("repertorio_mail", lRecord, "E") {

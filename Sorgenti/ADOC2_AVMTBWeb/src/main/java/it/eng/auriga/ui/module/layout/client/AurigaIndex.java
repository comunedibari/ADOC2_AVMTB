...
public class AurigaIndex extends Index {
	...
	public boolean getFlagRimuoviScript() {
        return getFlagRimuoviScript(null);
	}
	...
	protected void continuaOnModuleLoad(Dictionary dictionary, Record lRecordSchema){
		...
		if (skipLogin && (schemaSelezionato != null) && (!"".equalsIgnoreCase(schemaSelezionato))){
		...
		RPCManager.setHandleTransportErrorCallback(new HandleTransportErrorCallback() {			
			@Override
			public void handleTransportError(final int transactionNum, int status,
...
//  Federico Cacco
//	Vecchio codice commmentato, da togliere una volta sicuri che il multilingua funzioni
//	protected void setLoginNotRequired(final Record lRecordSchema) {
//		RPCManager.setLoginRequiredCallback(new LoginRequiredCallback() {
//			@Override
//			public void loginRequired(int transactionNum, RPCRequest request,
...
//	}
	...
	protected void setLoginRequired(final Record lRecordSchema) {
		RPCManager.setLoginRequiredCallback(new LoginRequiredCallback() {
			...
			public void loginRequired(int transactionNum, RPCRequest request,
				...
				if (lRecordSchema.getAttributeAsBoolean("showSelection")){
					showSelection(lRecordSchema.getAttributeAsRecordList("schemi"));
				} else {
	...
	/**
	 * Mostra la finestra di login. Con questa chiamata si assume che la login non sia dovuta
	 * alla scadenza della sessione, ma ad esempio per un nuovo avvio dell'applicazione
	 */
	...
	/**
	 * Mostra la finestra di login, parametrizzando se la nuova login è dovuta alla scadenza della sessione o ad
	 * altri motivi
	 * @param ignoraControlloLingua True se la login è richiesta per la scedenza della sessione
	 */
	...
	protected void showSelection(RecordList recordList) {
		if ((!lSchemaSelectionWindow.isDrawn()) || (!lSchemaSelectionWindow.isVisible())) {
			...
			lSchemaSelectionWindow.setSchema(lHashMap);
	...
	protected void proceedAsUsual(String schema) {
		...
		new GWTRestService<Object, Object>("ServiceRestPrivilegiFilterUtil").call(new Record(), new ServiceCallback<Record>() {
			@Override
			public void execute(Record object) {

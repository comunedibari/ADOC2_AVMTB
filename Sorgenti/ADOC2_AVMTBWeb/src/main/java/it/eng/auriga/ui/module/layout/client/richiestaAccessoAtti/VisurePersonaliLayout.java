...
public class VisurePersonaliLayout extends CustomLayout {
	...
	public VisurePersonaliLayout(String nomeEntita, String finalita, Boolean flgSelezioneSingola, Boolean showOnlyDetail) {
		super(nomeEntita, new GWTRestDataSource("VisurePersonaliDatasource"), 
	...
	protected MultiToolStripButton[] getMultiselectButtons() {
		...
		if(impostaRespIstruttoria == null) {
			impostaRespIstruttoria = new MultiToolStripButton("protocollazione/flagSoggettiGruppo/O2.png", this, I18NUtil.getMessages().visure_personali_impostaRespIstruttoriaButtonMassivo(),
				...
				@Override
				public boolean toShow() {
					return Layout.isPrivilegioAttivo("RAA/AUT");
				}
				...
				public void doSomething() {
					...
					final SelezioneRespIstruttoriaVisurePopup responsabiliIstruttorieVisurePopup = new SelezioneRespIstruttoriaVisurePopup(
						...
						public void onClickOkButton(final DSCallback callback) {
							...
							Record record = new Record();
							record.setAttribute("listaRecord", listaRecord);
							record.setAttribute("listaRespIstruttoria", _form.getValueAsRecordList("listaRespIstruttoria"));
							Layout.showWaitPopup("Impostazione responsabile istruttoria in corso: potrebbe richiedere qualche secondo. Attendere…");
							GWTRestDataSource lGwtRestDataSource = new GWTRestDataSource("SelezioneRespIstruttoriaVisureDataSource");
							try {
								lGwtRestDataSource.addData(record, new DSCallback() {
									...
									public void execute(DSResponse response, Object rawData, DSRequest request) {
										massiveOperationCallback(response, listaRecord, "idProcedimento", "numeroProposta",
												"Impostazione responsabile istruttoria effettuata con successo", "Tutti i record selezionati per l'impostazione responsabile istruttoria sono andati in errore!",
												"Alcuni dei record selezionati per l'impostazione responsabile istruttoria!", callback);
									}
								});
							} catch (Exception e) {
								Layout.hideWaitPopup();

...
public class NuovaAttiRichiestiCanvas extends ReplicableCanvas {
	...
	private final int TITTLE_ALIGN_WIDTH = 120;
	...
	public void disegna() {
		...
		lVLayoutCanvas.setMembers(estremiAttoRichiestoForm, statoAttoRichiestoForm, noteAttoRichiestoForm);
	...
	private void buildStatoAttoRichiestoForm(){
		...
		udcItem = new ExtendedTextItem("udc", setTitleAlign("UDC", TITTLE_ALIGN_WIDTH, false));
		udcItem.setStartRow(true);
		...
		selectCompetenzaDiUrbanisticaItem = new SelectItem("competenzaDiUrbanistica", "Competenza di Urbanistica");
		...
		selectCompetenzaDiUrbanisticaItem.setWidth(120);
		...
		selectCartaceoReperibileItem = new SelectItem("cartaceoReperibile", "Cartaceo reperibile");
		...
		selectCartaceoReperibileItem.setWidth(120);
		...
		flgRichiestaVisioneCartaceoItem = new CheckboxItem("flgRichiestaVisioneCartaceo", "Richiesta visione cartaceo");
		flgRichiestaVisioneCartaceoItem.setStartRow(false);
		flgRichiestaVisioneCartaceoItem.setColSpan(1);
		
		flgRichiestaVisioneCartaceoItem.setShowIfCondition(new FormItemIfFunction() {
			
			@Override
			public boolean execute(FormItem item, Object value, DynamicForm form) {
				String statoScansione = statoScansioneItem.getValue() != null ? (String) statoScansioneItem.getValue() : null;
				flgRichiestaVisioneCartaceoItem.setCanEdit(getEditing() && !(statoScansione != null && "N".equalsIgnoreCase(statoScansione)));
				return true;
			}
		});
		
		statoAttoRichiestoForm.setFields(udcItem, selectCompetenzaDiUrbanisticaItem, selectCartaceoReperibileItem, selectInArchivioItem, flgRichiestaVisioneCartaceoItem);		
	}
	...
	public ReplicableCanvasForm[] getForm() {
		return new ReplicableCanvasForm[] {estremiAttoRichiestoForm, statoAttoRichiestoForm, noteAttoRichiestoForm};
	}
	...
	public void setCanEdit(Boolean canEdit) {
		...
		udcItem.setCanEdit(canEdit && isTaskArchivio());
		
		noteUffRichiedenteItem.setCanEdit(canEdit && !isTaskArchivio());
		noteCittadellaItem.setCanEdit(canEdit && isTaskArchivio());
	...
	public void editRecordDaRecuperaDatiProtocollo(Record record, boolean aggiornaSoloStatoScansioni) {
		if(aggiornaSoloStatoScansioni){
			...
			if (!isRecordUgualiString(record, estremiAttoRichiestoForm.getValuesAsRecord(), "classifica") || !isRecordUgualiString(record, statoAttoRichiestoForm.getValuesAsRecord(), "competenzaDiUrbanistica", "cartaceoReperibile", "inArchivio", "udc")) {
				variazioni = true;
			}

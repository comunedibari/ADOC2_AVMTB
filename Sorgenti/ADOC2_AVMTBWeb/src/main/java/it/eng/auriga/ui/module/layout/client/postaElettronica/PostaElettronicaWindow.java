		...
		lGwtRestDataSource.performCustomOperation("get", lRecord, new DSCallback() {
			...
			public void execute(DSResponse response, Object rawData, DSRequest request) {
				if (response.getStatus() == DSResponse.STATUS_SUCCESS) {
					...
					DettaglioEmailWindow lDettaglioEmailWindow = null;
					if (customLayoutToDoSearch != null) {
						lDettaglioEmailWindow = new DettaglioEmailWindow(idEmail, tipoRel, null, lRecord, null, customLayoutToDoSearch);
					} else if (customDetailToReload != null) {
						lDettaglioEmailWindow = new DettaglioEmailWindow(idEmail, tipoRel, null, lRecord, null, customDetailToReload);
					} else {
						lDettaglioEmailWindow = new DettaglioEmailWindow(idEmail, tipoRel, null, lRecord, null);
					}
					if (callback != null) {

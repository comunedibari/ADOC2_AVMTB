...
public class LoginDataSource extends AbstractServiceDataSource<AurigaLoginBean, AurigaLoginBean> {
	...
	public AurigaLoginBean call(AurigaLoginBean bean) throws Exception {
		...
		try {
			...
			if (fromExtAppl) {
				...
				logLoginTry(bean, "PORTLET_AURIGAWEB", useridappl, output.isInError(), lLoginBean.getCodidconnectiontokenout());
				...
				String denominazione = (StringUtils.isNotBlank(lLoginBean.getDesuserout())) ? lLoginBean.getDesuserout() : userid;
				...
				logLoginTry(bean, null, userid, output.isInError(), lLoginBean.getCodidconnectiontokenout());
			...
			if (lista != null) {
				for (int i = 0; i < lista.getRiga().size(); i++) {
					mLogger.debug("ITERAZIONE : " + i );
					Vector<String> v = new XmlUtility().getValoriRiga(lista.getRiga().get(i));
					mLogger.debug("v.get(0) : "+v.get(0));
	...
	private void logLoginTry(AurigaLoginBean bean, String codApplicazione, String userid, boolean flagKO, String codIdConnectionToken) {
		...
		try {
			logLoginTryHelperCall.call(bean, codApplicazione, userid, flagKO, codIdConnectionToken, parametriClient);
		} catch (Exception e) {

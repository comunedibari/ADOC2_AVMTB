...
public class PostaElettronicaList extends CustomList {
	...
	public PostaElettronicaList(String nomeEntita, String pIdNode) {
		...
		iconaFlgSpamField.setCellFormatter(new CellFormatter() {
			...
			public String format(Object value, ListGridRecord record, int rowNum, int colNum) {
				...
				if (flgSpam != null && "1".equals(flgSpam)) {
					return buildIconHtml("postaElettronica/spam.png");
				}
				return null;
			}
		});
			...
			public String format(Object value, ListGridRecord record, int rowNum, int colNum) {
				...
				if (allegatiEmail != null && !"".equals(allegatiEmail)) {
					if (allegatiEmail.equals("A")) {
						...
						return buildImgButtonHtml("allegatiFirmati.png");
					}
				}
				return null;
			}
		...
		iconaFlgNoAssocAutoField.setCellFormatter(new CellFormatter() {
			...
			public String format(Object value, ListGridRecord record, int rowNum, int colNum) {
				...
				if (flgNoAssocAuto != null && "1".equals(flgNoAssocAuto)) {
					return buildIconHtml("warning.png");
				}
				return null;
			}
		});
		...
		iconaFlgRichConfermaField.setCellFormatter(new CellFormatter() {
			...
			public String format(Object value, ListGridRecord record, int rowNum, int colNum) {
				...
				if (flgRichConferma != null && "1".equals(flgRichConferma)) {
					return buildIconHtml("postaElettronica/richConferma.png");
				}
				return null;
			}
		});
		...
		iconaFlgInviataRispostaField.setCellFormatter(new CellFormatter() {
			...
			public String format(Object value, ListGridRecord record, int rowNum, int colNum) {
				...
				if (flgInviataRisposta != null && "1".equals(flgInviataRisposta)) {
					return buildIconHtml("postaElettronica/iconMilano/risposta_mail.png");
		...
		iconaFlgInoltrataField.setCellFormatter(new CellFormatter() {
			...
			public String format(Object value, ListGridRecord record, int rowNum, int colNum) {
				...
				if (flgInviataInoltro != null && "1".equals(flgInviataInoltro)) {
					...
					return buildIconHtml("postaElettronica/iconMilano/inoltro_mail.png");
				}
				return null;
			}
		});
		...
		iconaFlgStatoProtField.setCellFormatter(new CellFormatter() {
			...
			public String format(Object value, ListGridRecord record, int rowNum, int colNum) {
				...
				if (flgStatoProt != null && !"".equals(flgStatoProt)) {
					return buildIconHtml("postaElettronica/flgStatoProt/" + flgStatoProt + ".png");
				}
				return null;
			}
		});
		...
		emailCollegataField.addRecordClickHandler(new RecordClickHandler() {
			...
			public void onRecordClick(RecordClickEvent event) {
				...
				Record record = event.getRecord();
				if (record.getAttribute("idEmailCollegata") != null && !"".equals(record.getAttribute("idEmailCollegata"))) {
					DettaglioEmailWindow lDettaglioEmailWindow = new DettaglioEmailWindow(record.getAttribute("idEmailCollegata"), null, getLayout());
				}
			}
		});
		...
		statoLavorazioneField.setCellFormatter(new CellFormatter() {
			...
			public String format(Object value, ListGridRecord record, int rowNum, int colNum) {
				...
				if (statoLavorazione != null && !"".equals(statoLavorazione)) {
					return buildIconHtml("postaElettronica/statoLavorazione/" + statoLavorazione + ".png");
				}
				return null;
			}
		});
	...
	protected void visualizzaFile(final Record detailRecord, final String display, final String uri, final String remoteUri, InfoFileRecord info) {
		...
		if (info != null && info.getMimetype() != null && info.getMimetype().startsWith("image") && !info.getMimetype().equals("image/tiff")) {
			PreviewImageWindow lPreviewImageWindow = new PreviewImageWindow(uri, Boolean.valueOf(remoteUri), info);
		} else {
	...
	private void retrieveAllegati(Record record, final RetrieveInfoFileCallback pRetrieveInfoFileCallback) {
		...
		if (lMapEmailDetailRecord.containsKey(idEmail)) {
			...
			GWTRestService<Object, Object> lGwtRestService = new GWTRestService<Object, Object>("AurigaAbilitazioniEmailDataSource");
			lGwtRestService.call(record, new ServiceCallback<Record>() {
				...
				public void execute(Record detailRecord) {
					...
					if (detailRecord != null){
						...
						lGwtRestDataSource.executecustom("retrieveAttach", detailRecord, new DSCallback() {
	...
	protected void showRowContextMenu(final Record pRecord) {
		GWTRestService<Object, Object> lGwtRestService = new GWTRestService<Object, Object>("AurigaAbilitazioniEmailDataSource");
		lGwtRestService.call(pRecord, new ServiceCallback<Record>() {
			...
			public void execute(final Record object) {
				...
				recordAzioneRapidaMessaInCarico.setAttribute("azioneRapida", AzioniRapide.MANDA_IN_APPROVAZIONE.getValue()); 
				listaAzioniRapide.add(recordAzioneRapidaMessaInCarico);
				Record recordAzioneRapidaMandaInApprovazione = new Record();
				recordAzioneRapidaMandaInApprovazione.setAttribute("azioneRapida", AzioniRapide.ASSEGNA_UO_COMPETENTE.getValue()); 
	...
	protected void buildMenu(final Record abilitazioni, final Record pRecordMail, final Record destinatariPreferiti) {
		...
		if( (abilitazioni.getAttribute("abilitaRepertoria") != null && abilitazioni.getAttribute("abilitaRepertoria").equals("true")) && 
			(abilitazioni.getAttribute("abilitaProtocolla") != null && abilitazioni.getAttribute("abilitaProtocolla").equals("true")) ) {
			...
			protocollaMenuItem.addClickHandler(new com.smartgwt.client.widgets.menu.events.ClickHandler() {
				...
				@Override
				public void onClick(MenuItemClickEvent event) {
					manageRegistraProtocollaClick(pRecordMail, destinatariEAccount);
				}
			});
			...
			repertorioMenuItem.addClickHandler(new com.smartgwt.client.widgets.menu.events.ClickHandler() {
				...
				@Override
				public void onClick(MenuItemClickEvent event) {
					//Aggiungere classi Daniele
					registraRepertorioClick(pRecordMail, destinatariEAccount);
				}
			});
		...
		} else if((abilitazioni.getAttribute("abilitaRepertoria") != null && abilitazioni.getAttribute("abilitaRepertoria").equals("false")) && 
				(abilitazioni.getAttribute("abilitaProtocolla") != null && abilitazioni.getAttribute("abilitaProtocolla").equals("true"))) {
		...
		} else if((abilitazioni.getAttribute("abilitaRepertoria") != null && abilitazioni.getAttribute("abilitaRepertoria").equals("true")) && 
				(abilitazioni.getAttribute("abilitaProtocolla") != null && abilitazioni.getAttribute("abilitaProtocolla").equals("false"))) {
		...
		if (allegatiEmail != null && !"".equals(allegatiEmail) && (allegatiEmail.equals("A") || allegatiEmail.equals("AF"))) {
			MenuItem scaricaZipAllegati = new MenuItem(I18NUtil.getMessages().posta_elettronica_list_scaricaZipAllegati(), "buttons/export.png");
											...
											sbloccaEmail(pRecordMail, new ServiceCallback<Record>() {
												...
												public void execute(Record data) {
													...
													if (errorMessages != null && errorMessages.size() > 0) {
														...
														record.setAttribute("listaRecord", listaEmail);
														final GWTRestService<Object, Object> lGwtRestServiceInvio = new GWTRestService<Object, Object>(
	...
	protected void manageProtocollaClick(final Record pRecord, final String destinatariEAccount, final String idTipoDoc, final String nomeTipoDoc) {
		...
		lGwtRestDataSource.performCustomOperation("get", pRecord, new DSCallback() {
						...
						SC.ask("Sono stati trovati dei file nella sezione Appunti & Note, vuoi aggiungerli come allegati alla protocollazione?", new BooleanCallback() {					
							...
							public void execute(Boolean value) {
								...
								if(value) {
									...
									pRecord.setAttribute("filedaAppunti",filedaAppunti); //Aggiunta della lista dei file recuperati alla protocollazione.
	...
	protected void manageStampaAllegati(final Record record) {
		...
		GWTRestDataSource lGwtRestDataSource = new GWTRestDataSource("AurigaGetDettaglioPostaElettronicaDataSource", "idEmail", FieldType.TEXT);
		lGwtRestDataSource.performCustomOperation("retrieveAttach", recordToLoad, new DSCallback() {
...
//		});
	...
	protected void manageInvioNotificaInteropClick(Record pRecord, String tipoNotifica) {
		final GWTRestService<Object, Object> lGwtRestServiceInvioNotifica = new GWTRestService<Object, Object>("AurigaInvioNotificaInteropDatasource");
	...
	protected void managerInvioNotificaInteropResponse(GWTRestService<Object, Object> lGwtRestServiceInvioNotifica, Record record) {
...
//				final String tipoDestinatarioPreferito = currentRecord.getAttribute("tipoDestinatarioPreferito");
	...
	public void manageLoadDetail(final Record record, final int recordNum, final DSCallback callback) {
		...
		if (!AurigaLayout.getParametroDBAsBoolean("DETT_EMAIL_UNICO")) {
			layout.changeDetail((GWTRestDataSource) getDataSource(), new PostaElettronicaDetail("posta_elettronica"));
...
//		buttonsFields.add(0, altreOpButtonField);
	...
	private void manageRegistraProtocollaClick(final Record pRecordMail, final String destinatariEAccount) {
		if(!AurigaLayout.getImpostazioniDocumentoAsBoolean("skipSceltaTipologiaEntrata")) {
			...
			AurigaLayout.apriSceltaTipoDocPopup(false, idTipoDocDefault, "PG", null, new ServiceCallback<Record>() {
				...
				public void execute(Record lRecordTipoDoc) {
					...
					manageProtocollaClick(pRecordMail, destinatariEAccount, idTipoDoc, nomeTipoDoc);
				}
			});
		} else {
	...
	private void registraRepertorioClick(final Record recordMail, final String destinatariEAccount) {
		...
		if (AurigaLayout.getImpostazioniDocumentoAsBoolean("skipSceltaRepertorioEntrata")) {
			if (AurigaLayout.getImpostazioniDocumentoAsBoolean("skipSceltaTipologiaRepertorioEntrata")) {
				...
				manageRegistraRepertorio(recordMail,destinatariEAccount,idTipoDoc,nomeTipoDoc,repertorioEntrata);
			} else {
					...
					if (repertorio != null && !"".equals(repertorio)) {
						AurigaLayout.apriSceltaTipoDocPopup(true, null, "R", repertorio, new ServiceCallback<Record>() {
							...
							public void execute(Record lRecordTipoDoc) {
								String idTipoDoc = lRecordTipoDoc != null ? lRecordTipoDoc.getAttribute("idTipoDocumento") : null;
								String nomeTipoDoc = lRecordTipoDoc != null ? lRecordTipoDoc.getAttribute("descTipoDocumento") : null;
								manageRegistraRepertorio(recordMail,destinatariEAccount,idTipoDoc,nomeTipoDoc,repertorio);
							}
						});
					}
				}
	...
	private void manageRegistraRepertorioCallback(Record lRecord, String destinatariEAccount,
						String idTipoDoc, String nomeTipoDoc, String repertorio){
		if (destinatariEAccount != null) {
			lRecord.setAttribute("casellaEmailDestinatario", destinatariEAccount); // viene settato il campo casellaEmailDestinatario del bean
		}

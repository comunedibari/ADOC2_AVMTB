...
public class NuovoMessaggioWindow extends PostaElettronicaWindow {
	...
	public NuovoMessaggioWindow(String tipoRel, DSCallback callback) {
		super("nuovo_messaggio", true, tipoRel, null, null, false, null, null, null, callback);

		_window = this;
	}

	public NuovoMessaggioWindow(String tipoRel, CustomLayout customLayoutToDoSearch, DSCallback callback) {
		super("nuovo_messaggio", true, tipoRel, null, null, false, null, null, null, customLayoutToDoSearch, callback);

		_window = this;
	}

	public NuovoMessaggioWindow(String tipoRel, CustomDetail customDetailToReload, DSCallback callback) {
	...
	private void buildNuovoMessaggio() {
		...
		_layout = new InvioMailLayoutNew(this, finalita, vm, null) {
			@Override
						...
						modelliDS.fetchData(criteria, new DSCallback() {
							...
							public void execute(DSResponse response, Object rawData, DSRequest request) {
								...
								if (data.length != 0) {
									...
									//TODO NUOVO CODICE DA MANTENERE AL POSTO DEL PRECEDENTE PER SALVARE I VALORI CORRETTI?
									/*
									 * Scasiono il values che è ritornato per verificare cosa c'è di nuovo e
									 * invece cosa deve essere mantenuto dal form precedente 
									 */
	//								int valuesLength = values.toMap().keySet().toArray().length;
	//								for(int indexValues = 0; indexValues < valuesLength; indexValues++){
	//									if(values.toMap().keySet().toArray()[indexValues] != null){
	//										if(values.toMap().get(values.toMap().keySet().toArray()[indexValues]) != null){
	//											
	//											//Allora imposto nei values del form quanto è arrivato
	//											oldValues.setAttribute(values.toMap().keySet().toArray()[indexValues].toString(), values.toMap().get(values.toMap().keySet().toArray()[indexValues]));
	//											
	//										}
	//									}
	//								}
	//								
	//								editNewRecord(oldValues.toMap());
								}
							}
						});
					}

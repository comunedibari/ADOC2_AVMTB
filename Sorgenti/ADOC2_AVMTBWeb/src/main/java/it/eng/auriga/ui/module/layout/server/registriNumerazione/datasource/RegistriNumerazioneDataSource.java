...
public class RegistriNumerazioneDataSource extends AbstractFetchDataSource<RegistriNumerazioneBean>  {
	...
	@Override
	public PaginatorBean<RegistriNumerazioneBean> fetch(AdvancedCriteria criteria, Integer startRow, Integer endRow, List<OrderByBean> orderby)
			throws Exception {
		...
		String flgStatoRegistro = null;


		String idTipoRegNum = null;

		if (criteria != null && criteria.getCriteria() != null) {
			for (Criterion crit : criteria.getCriteria()) {
				if ("codCategoria".equals(crit.getFieldName())) {
					codCategoria = getTextFilterValue(crit);
				...
				} else if ("flgStatoRegistro".equals(crit.getFieldName())) {
					flgStatoRegistro = getTextFilterValue(crit);
				}
			}
		}
		...
		input.setIdtiporegnumio(idTipoRegNum != null? new BigDecimal(idTipoRegNum) : null);
		input.setCodidconnectiontokenin(token);
		input.setFlgsenzapaginazionein(1);
		input.setIduserlavoroin(StringUtils.isNotBlank(idUserLavoro) ? new BigDecimal(idUserLavoro) : null);
		input.setCodcategoriaio(codCategoria);
		input.setSiglaio(sigla);
		input.setDescrizioneio(descrizione);
		input.setFlgstatoregistroio(flgStatoRegistro);
		...
		DmpkTipiRegNumUdTrovatipiregnum dmpkTipiRegNumUdTrovatipiregnum = new DmpkTipiRegNumUdTrovatipiregnum();
		StoreResultBean<DmpkTipiRegNumUdTrovatipiregnumBean> lStoreResultBean = dmpkTipiRegNumUdTrovatipiregnum.execute(getLocale(), loginBean, input);
		...
		if(lStoreResultBean.getDefaultMessage() != null) {
			...
			if(showErrorMsg) {
				addMessage("Errore nel recupero dell'output: " + lStoreResultBean.getDefaultMessage(), "", MessageType.ERROR);
			}
		} else {
			String xmlLista = lStoreResultBean.getResultBean().getListaxmlout();
			List<RegistriNumerazioneBean> lista = XmlListaUtility.recuperaLista(xmlLista, RegistriNumerazioneBean.class);
	...
	public RegistriNumerazioneBean get(RegistriNumerazioneBean bean) throws Exception {
		...
		result.setSigla(output.getResultBean().getSiglaout());
		result.setTsInizioStatoRegistrazione(output.getResultBean().getTsiniziostatoregistroout() != null ? 
				new SimpleDateFormat(FMT_STD_TIMESTAMP).parse(output.getResultBean().getTsiniziostatoregistroout()) : null );
		...
		result.setPeriodicitaDiAnni(output.getResultBean().getPeriodicitadianniout() != null ? output.getResultBean().getPeriodicitadianniout().toString() : null);
		result.setAnnoInizioNumerazione(output.getResultBean().getAnnoinizionumerazioneout() != null ? output.getResultBean().getAnnoinizionumerazioneout().toString() : null);
		result.setCiProvTipoRegNum(output.getResultBean().getCiprovtiporegnumout());
	...
	public RegistriNumerazioneBean add(RegistriNumerazioneBean bean) throws Exception {
		...
		log.info("start save Registri numerazione");
		...
		input.setCodidconnectiontokenin(token);
		input.setIduserlavoroin(StringUtils.isNotBlank(idUserLavoro) ? new BigDecimal(idUserLavoro) : null);
		input.setRowidout(bean.getRowId());
//		input.setIdtiporennumio(bean.getIdTipoRegNum() != null ? new BigDecimal(bean.getIdTipoRegNum()) : null);
		input.setDescrizionein(bean.getDescrizione());
		input.setCodcategoriain(bean.getCodCategoria());
		input.setSiglain(bean.getSigla());
		input.setDtiniziovldin(bean.getDtInizioVld() != null ? bean.getDtInizioVld().toString() : null);
		input.setDtfinevldin(bean.getDtFineVld() != null ? bean.getDtFineVld().toString() : null);
		input.setFlginternain(bean.getFlgInterna() != null && bean.getFlgInterna() ? 1 : 0);
		input.setFlgrichabilxassegnin(bean.getFlgRichAbilXAssegn() != null && bean.getFlgRichAbilXAssegn() ? 1 : 0);
		input.setFlgrichabilxvisualizzin(bean.getFlgRichAbilVis() != null && bean.getFlgRichAbilVis() ? 1 : 0);
		input.setFlgrichabilxfirmain(bean.getFlgAbilFirma() != null && bean.getFlgAbilFirma() ? 1 : 0);
		input.setFlgrichabilxgestin(bean.getFlgRichAbilXGestIn() != null && bean.getFlgRichAbilXGestIn() ? 1 : 0);

		if (bean.getFlgStatoRegistro() != null) {
			if(bean.getFlgStatoRegistro().equalsIgnoreCase("aperto")) {
				input.setFlgstatoregistroin("A");
			}else {
				input.setFlgstatoregistroin("C");
			}
		}else {
			input.setFlgstatoregistroin(null);
		}

		input.setPeriodicitadianniin(bean.getPeriodicitaDiAnni() != null ? Integer.parseInt(bean.getPeriodicitaDiAnni()) : null);
		input.setAnnoinizionumerazionein(bean.getAnnoInizioNumerazione() != null ? Integer.parseInt(bean.getAnnoInizioNumerazione()) : null);
		input.setCiprovtiporegnumin(bean.getCiProvTipoRegNum());

		if (bean.getFlgAmmEscXTipiDoc() != null) {
			if(bean.getFlgAmmEscXTipiDoc().equalsIgnoreCase("ammessi")) {
				input.setFlgstatoregistroin("A");
			}else {
				input.setFlgstatoregistroin("E");
			}
		}else {
			input.setFlgstatoregistroin(null);
		}
		...
		log.info("end save registri numerazione");
		return bean;
	}

	@Override
	public RegistriNumerazioneBean update(RegistriNumerazioneBean bean, RegistriNumerazioneBean oldvalue)
			throws Exception {
		...
		input.setFlgrichabilxfirmain(bean.getFlgAbilFirma() != null && bean.getFlgAbilFirma() ? 1 : 0);
		input.setFlgrichabilxgestin(bean.getFlgRichAbilXGestIn() != null && bean.getFlgRichAbilXGestIn() ? 1 : 0);
		input.setFlgstatoregistroin(bean.getFlgStatoRegistro() != null ? bean.getFlgStatoRegistro() : null);
		input.setPeriodicitadianniin(bean.getPeriodicitaDiAnni() != null ? Integer.parseInt(bean.getPeriodicitaDiAnni()) : null);
		input.setAnnoinizionumerazionein(bean.getAnnoInizioNumerazione() != null ? Integer.parseInt(bean.getAnnoInizioNumerazione()) : null);
		input.setCiprovtiporegnumin(bean.getCiProvTipoRegNum());
		input.setFlgammescxtipidocin(bean.getFlgAmmEscXTipiDoc() != null ? bean.getFlgAmmEscXTipiDoc() : null);
		input.setFlginternain(bean.getFlgInterna() != null && bean.getFlgInterna() ? 1 : 0);
		input.setAnnoinizionumerazionein(bean.getAnnoInizioNumerazione() != null ? Integer.parseInt(bean.getAnnoInizioNumerazione()) : null);
	...
	public RegistriNumerazioneBean remove(RegistriNumerazioneBean bean) throws Exception {
		...
		log.info("start remove Registri Numerazione");
		...
		log.info("end remove tipologia documentale");
		return bean;
	}

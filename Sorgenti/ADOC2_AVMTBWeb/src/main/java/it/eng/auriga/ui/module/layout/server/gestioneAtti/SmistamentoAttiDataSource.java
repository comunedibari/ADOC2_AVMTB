...
public class SmistamentoAttiDataSource extends AbstractDataSource<SmistamentoAttiBean, SmistamentoAttiBean>{	
	...
	public SmistamentoAttiBean add(SmistamentoAttiBean bean) throws Exception {		
		...
		for(AttiBean atto : bean.getListaRecord()) {						
			...
			if(StringUtils.isNotBlank(atto.getRuoloSmistamento())) {
				...
				if(atto.getIdProcedimento()!=null)
					input.setIdprocessin(new BigDecimal(atto.getIdProcedimento()));
				
				input.setRuolosoggettiin(atto.getRuoloSmistamento());
				
				input.setListaxmlin(getXmlSmistamento(bean));
				...
				if(output.getDefaultMessage() != null) {
					...
					errorMessages.put(atto.getIdProcedimento(), output.getDefaultMessage());
				}			
			} else {
				if(errorMessages == null) errorMessages = new HashMap<String, String>();
				errorMessages.put(atto.getIdProcedimento(), "Lo stato dell’atto e/o le tue abilitazioni non ti consentono di smistarlo");
			}
	...
	public String getXmlSmistamento(SmistamentoAttiBean bean) throws Exception {

...
public class AccountInvioEmailDatasource extends AbstractFetchDataSource<SimpleKeyValueBean> {
	...
	// lListaIdCaselleInput.setIdUtente(lAurigaLoginBean.getSpecializzazioneBean().getIdUtenteModPec());
	// lListaIdCaselleInput.setIdEnteAOO(lAurigaLoginBean.getSpecializzazioneBean().getIdDominio()+"");
	...
	// List<SimpleKeyValueBean> lListAccount = new ArrayList<SimpleKeyValueBean>();
	// String tipoMail = getExtraparams().get("tipoMail");
	// if (StringUtils.isNotBlank(tipoMail)){
	// List<InfoCasella> lListaAccount = lCasellaUtility.getlistacaselleutente(getLocale(), lListaIdCaselleInput).getAccounts();
	// for (InfoCasella lInfoCasella : lListaAccount){
	// if (tipoMail == null || (tipoMail.equals("PEC") && lInfoCasella.getIsPec()) || (tipoMail.equals("PEO") && !lInfoCasella.getIsPec())) {
	// String idAccount=lInfoCasella.getIdAccount();
	...
	// filtroAccount.setIdAccount(idAccount);
	...
	// String account = listaAccount.get(0).getAccount();
	// SimpleKeyValueBean lSimpleKeyValueBean = new SimpleKeyValueBean();
	// lSimpleKeyValueBean.setKey(account);
	// lSimpleKeyValueBean.setValue(account);
	// lListAccount.add(lSimpleKeyValueBean);
	// }
	// }
	// } else {
	// List<InfoCasella> lListaAccount = lCasellaUtility.getlistacaselleutente(getLocale(), lListaIdCaselleInput).getAccounts();
	// List<SimpleKeyValueBean> listaPeo = new ArrayList<SimpleKeyValueBean>();
	// List<SimpleKeyValueBean> listaPec = new ArrayList<SimpleKeyValueBean>();
	// for (InfoCasella lInfoCasella : lListaAccount){
	// String idAccount=lInfoCasella.getIdAccount();
	// TFilterFetch<MailboxAccountBean> filterfetch = new TFilterFetch<MailboxAccountBean>();
	// MailboxAccountBean filtroAccount = new MailboxAccountBean();
	...
	// lSimpleKeyValueBean.setKey(account);
	// lSimpleKeyValueBean.setValue(account);
	// if(lInfoCasella.getIsPec()) {
	// listaPec.add(lSimpleKeyValueBean);
	// } else {
	// listaPeo.add(lSimpleKeyValueBean);
	// }
	// }
	// lListAccount.addAll(listaPeo);
	// lListAccount.addAll(listaPec);
	// // Map<String, String> lMapAccount = lCasellaUtility.getlistacaselleutentewithaccount(getLocale(), lListaIdCaselleInput).getAccounts();
	// // for (String idAccount : lMapAccount.keySet()){
	// // SimpleKeyValueBean lSimpleKeyValueBean = new SimpleKeyValueBean();
	// // String account = lMapAccount.get(idAccount);
	// // lSimpleKeyValueBean.setKey(account);
	// // lSimpleKeyValueBean.setValue(account);
	// // lListAccount.add(lSimpleKeyValueBean);
	// // }
	// }
	// lPaginatorBean.setData(lListAccount);
	// lPaginatorBean.setEndRow(lListAccount.size());
	// lPaginatorBean.setStartRow(0);
	// lPaginatorBean.setTotalRows(lListAccount.size());

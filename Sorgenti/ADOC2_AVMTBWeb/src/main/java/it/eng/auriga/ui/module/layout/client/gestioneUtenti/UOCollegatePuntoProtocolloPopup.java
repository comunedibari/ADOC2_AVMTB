...
public abstract class UOCollegatePuntoProtocolloPopup extends ModalWindow {
	...
	// GridItem
	private ListGrid uoPuntoProtocolloGrid;
	...
	public UOCollegatePuntoProtocolloPopup(Record record, String title, String mode){
		...
		okButton.addClickHandler(new com.smartgwt.client.widgets.events.ClickHandler() {			
			...
			public void onClick(com.smartgwt.client.widgets.events.ClickEvent event) {
				if(formMain.validate()) {
					...
					for (ListGridRecord r : uoPuntoProtocolloGrid.getRecords()) {						
		...
		layout.addMember(uoPuntoProtocolloGrid);
	...
	private void disegnaForm(Record record) {
		...
		// LISTA UO
		uoPuntoProtocolloGrid = new ListGrid();
		uoPuntoProtocolloGrid.setWidth100();
		uoPuntoProtocolloGrid.setHeight100();  
		uoPuntoProtocolloGrid.setShowHeader(true);
		uoPuntoProtocolloGrid.setAlternateRecordStyles(false);
		uoPuntoProtocolloGrid.setLeaveScrollbarGap(false);		
		uoPuntoProtocolloGrid.setKeepInParentRect(true);
		uoPuntoProtocolloGrid.setLoadingMessage(null);
		uoPuntoProtocolloGrid.setDataFetchMode(FetchMode.LOCAL);		
		uoPuntoProtocolloGrid.setShowEmptyMessage(false);
		uoPuntoProtocolloGrid.setShowRollOver(false);		
		uoPuntoProtocolloGrid.setCanReorderFields(false);
		uoPuntoProtocolloGrid.setCanResizeFields(false);
		uoPuntoProtocolloGrid.setCanReorderRecords(false);
		uoPuntoProtocolloGrid.setCanHover(true);		
		uoPuntoProtocolloGrid.setCanGroupBy(false);	
		uoPuntoProtocolloGrid.setCanSort(false);
		uoPuntoProtocolloGrid.setShowHeaderContextMenu(false);
		uoPuntoProtocolloGrid.setNoDoubleClicks(true); 
		uoPuntoProtocolloGrid.setVirtualScrolling(true);      
		uoPuntoProtocolloGrid.setCanEdit(isEditMode());
		uoPuntoProtocolloGrid.setSelectionType(SelectionStyle.NONE);  
		
		idUOField            = new ListGridField("idUO", "Id. UO");                      idUOField.setHidden(true);		
		livelliUOField       = new ListGridField("livelliUO", "Codice UO");              livelliUOField.setWidth(200);
		denominazioneUOField = new ListGridField("denominazioneUO", "Denominazione UO"); denominazioneUOField.setWidth(400);
		flgAbilitazioneField = new ListGridField("flgAbilitazione", "Abilitato");        flgAbilitazioneField.setType(ListGridFieldType.BOOLEAN);  
		
		RecordList listaUOCollegatePuntoProtocolloRec = record.getAttributeAsRecordList("listaUOCollegatePuntoProtocollo");
		
        uoPuntoProtocolloGrid.setData(listaUOCollegatePuntoProtocolloRec);
		
		uoPuntoProtocolloGrid.setFields(idUOField, livelliUOField, denominazioneUOField, flgAbilitazioneField);

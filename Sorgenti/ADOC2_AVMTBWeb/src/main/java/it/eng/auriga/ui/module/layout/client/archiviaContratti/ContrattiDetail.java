...
import it.eng.auriga.ui.module.layout.client.firma.InfoFirmaWindow;
...
public class ContrattiDetail extends CustomDetail {
	...
	private void buildContenutiForm() {
		...
		filePrimarioButtons.setNestedFormItems(uploadFilePrimarioItem, previewButton, previewEditButton, editButton, fileFirmatoDigButton,
				firmaNonValidaButton, altreOpButton);
	...
	public static void drawFirmaWindow(InfoFileRecord pInfoFileRecord, String uri, boolean remoteUri, final boolean showValidita, String dataRif) {
		if (pInfoFileRecord.isFirmato() && pInfoFileRecord.getInfoFirma() != null && !pInfoFileRecord.getInfoFirma().equals("")) {
			InfoFirmaWindow lInfoFirmaWindow = new InfoFirmaWindow(pInfoFileRecord, showValidita);
			lInfoFirmaWindow.show();
		} else {
			GWTRestService<Object, Object> lGwtRestService = new GWTRestService<Object, Object>("CalcolaFirmaDatasource");
			Record lRecord = new Record();
			Record record = new Record();
			record.setAttribute("uri", uri);
			record.setAttribute("remote", remoteUri);
			record.setAttribute("dataRif", dataRif);
			lRecord.setAttribute("value", record);
			lGwtRestService.call(lRecord, new ServiceCallback<Record>() {

				@Override
				public void execute(Record object) {
					
					InfoFileRecord info = InfoFileRecord.buildInfoFileRecord(object.getJsObj());
					InfoFirmaWindow lInfoFirmaWindow = new InfoFirmaWindow(info, showValidita);
					lInfoFirmaWindow.show();
				}
			});
	...
	private void editFile(InfoFileRecord info, String display, String uri, Boolean remoteUri) {
		...
		if (estensione.equalsIgnoreCase("p7m")) {
			...
			new GWTRestService<Object, Object>("SbustaFileService").call(lRecordDaSbustare, new ServiceCallback<Record>() {
	...
	protected void manageFileFirmatoButtonClick() {
		...
		informazioniFirmaMenuItem.addClickHandler(new com.smartgwt.client.widgets.menu.events.ClickHandler() {
			...
			public void onClick(MenuItemClickEvent event) {
				...
				// boolean remoteUri = remote==null||remote.trim().equals("")||remote.trim().equals("false")?false:true;
				// Date lDate = dataProtocolloItem.getValueAsDate();
				// String lStrDate = DateUtil.format(lDate);
				// drawFirmaWindow(lInfoFileRecord, (String)uriFilePrimarioItem.getValue(), remoteUri, false , lStrDate);

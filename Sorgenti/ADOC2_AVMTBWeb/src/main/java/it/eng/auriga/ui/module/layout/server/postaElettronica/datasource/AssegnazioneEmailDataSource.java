...
import it.eng.auriga.ui.module.layout.server.postaElettronica.datasource.bean.AssegnatariEmailBean;
...
public class AssegnazioneEmailDataSource extends AbstractDataSource<AssegnazioneEmailBean, AssegnazioneEmailBean> {
	...
	private List<AssegnatariEmailBean> getListaAssegnatari(AssegnazioneEmailBean bean) throws Exception {
		List<AssegnatariEmailBean> listaAssegnatari = new ArrayList<AssegnatariEmailBean>();
		if (bean.getListaAssegnazioni() != null) {
			for (AssegnazioneBean assegnazioneBean : bean.getListaAssegnazioni()) {
				AssegnatariEmailBean lAssegnatariEmailBean = new AssegnatariEmailBean();
				lAssegnatariEmailBean.setTipo(getTipoAssegnatario(assegnazioneBean));
				if(lAssegnatariEmailBean.getTipo() != null && lAssegnatariEmailBean.getTipo().equals(TipoAssegnatario.GRUPPO)) {								
					lAssegnatariEmailBean.setIdSettato(assegnazioneBean.getGruppo());										
				} else {					
					lAssegnatariEmailBean.setIdSettato(assegnazioneBean.getIdUo());					
				}	
				//TODO ASSEGNAZIONE EMAIL SAVE OK
				lAssegnatariEmailBean.setMessaggio(bean.getMessaggio());
				if(StringUtils.isNotBlank(lAssegnatariEmailBean.getIdSettato())) {
					listaAssegnatari.add(lAssegnatariEmailBean);
				}
			}
		}

...
//					if (listaPKC==null || listaPKC.length==0)
				...
				LogWriter.writeLog("ProprietÃ  " + PreferenceKeys.PROPERTY_SIGN_CODECHECKENABLED + "=" + codeCheck );
				if( codeCheck ){
				...
				/*
				 * Controllo se il certificato è null o meno.
				 * Il certificato può essere null ad esempio se il provider che si sta utilizzando non lo riconosce
				 */
				if(certificate != null)
				{
	...
	//										} else {
	//											LogWriter.writeLog("Il file Ã¨ stato giÃ  firmato dal CN subject certificato " );
	//											panelSign.getTextArea().append("Il file " + hash.getFileName() + " Ã¨ stato giÃ  firmato dal CN subject certificato.\n");
	//										}
										}
									}
								}						
	...
	//							panelSign.showMessageDialog( messaggio, "",JOptionPane.INFORMATION_MESSAGE );
				...
				if( exceptionMessage.equalsIgnoreCase(Messages.getMessage( MessageKeys.MSG_FIRMAMARCA_ERROR_PINLOCKED ))){
					...
					break; 
					/*
					 * Il break è stato inserito perchè altrimenti in caso venga generata un'eccezione, ad esempio quando viene inserito un pin errato,

...
public class SmartCard implements ISmartCard{
	...
	public void init() {
		...
		try{
			...
			try {
				...
				LogWriter.writeLog("VerrÃ  utilizzato il fileOutputProvider " + fileOutputProviderClass);
			} catch (Exception e) {
				LogWriter.writeLog("Errore - Non Ã¨ stato possibile recuperare il fileOutputProvider da utilizzare", e );
			}
			if( fileOutputProviderClass==null ){
				LogWriter.writeLog("Errore - Non Ã¨ stato configurato il fileOutputProvider da utilizzare");
				//return;
			} else {
				try {
	...
	public FileBean buildFileInfo(FileBean bean) throws Exception{
		...
		if(FileUtility.isPdf( bean.getFile() )){
			LogWriter.writeLog("Il file da firmare Ã¨ PDF");
			bean.setIsPdf( true);
		} else {
			LogWriter.writeLog("Il file da firmare non Ã¨ PDF");
			bean.setIsPdf( false );
		}

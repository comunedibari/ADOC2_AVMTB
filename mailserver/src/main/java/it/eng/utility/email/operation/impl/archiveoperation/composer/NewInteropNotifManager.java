...
public class NewInteropNotifManager extends AbstractInteropNotifManager {

	private Logger log = LogManager.getLogger(NewInteropNotifManager.class);

	/**
	 * Metodo di gestione di una mail interoperabile, nuova versione
	 * 
	...
	public EmailBean notifInteropManage(AbstractInterbean busta, EmailBean emailBean, String idEmailInProcess, String accountDestinatarioEmailInProcess,
		...
		try {
			...
			if (((InterBean) busta).getIsdtdValid()
					&& (((InterBean) busta).getErroriConformita() == null || ((InterBean) busta).getErroriConformita().size() == 0)) {

				log.debug("Avvio gestione relazioni notifiche interoperabili");

				switch (((InterBean) busta).getType()) {
				case CONFERMA_RICEZIONE:
					if (((InterBean) busta).getConfermaricezione() != null && ((InterBean) busta).getConfermaricezione().getMessaggioRicevuto() != null
							&& ((InterBean) busta).getConfermaricezione().getMessaggioRicevuto().getIdentificatore() != null) {
						...
						numeroRegistrazioneInvioS = XmlFieldUtil.cleanField(
								((InterBean) busta).getConfermaricezione().getMessaggioRicevuto().getIdentificatore().getNumeroRegistrazione().getContent());
						dataRegistrazioneInvioS = XmlFieldUtil.cleanField(
								((InterBean) busta).getConfermaricezione().getMessaggioRicevuto().getIdentificatore().getDataRegistrazione().getContent());
						siglaRegistroInvio = XmlFieldUtil.cleanField(
								((InterBean) busta).getConfermaricezione().getMessaggioRicevuto().getIdentificatore().getCodiceRegistro().getContent());
						codiceAOOInvio = XmlFieldUtil
								.cleanField(((InterBean) busta).getConfermaricezione().getMessaggioRicevuto().getIdentificatore().getCodiceAOO().getContent());
						codiceAmministrazioneInvio = XmlFieldUtil.cleanField(
								((InterBean) busta).getConfermaricezione().getMessaggioRicevuto().getIdentificatore().getCodiceAmministrazione().getContent());
					} else {
						busta.addErroreXML(AbstractInterbean.ERRORE_XML_MESSAGGIO_RICEVUTO);
					}
					if (((InterBean) busta).getConfermaricezione() != null && ((InterBean) busta).getConfermaricezione().getIdentificatore() != null) {
						// identificatore del protocollo ricevuto
						numeroRegistrazioneRicezioneS = XmlFieldUtil
								.cleanField(((InterBean) busta).getConfermaricezione().getIdentificatore().getNumeroRegistrazione().getContent());
						dataRegistrazioneRicezioneS = XmlFieldUtil
								.cleanField(((InterBean) busta).getConfermaricezione().getIdentificatore().getDataRegistrazione().getContent());
						siglaRegistroRicezione = XmlFieldUtil
								.cleanField(((InterBean) busta).getConfermaricezione().getIdentificatore().getCodiceRegistro().getContent());
						codiceAOORicezione = XmlFieldUtil
								.cleanField(((InterBean) busta).getConfermaricezione().getIdentificatore().getCodiceAOO().getContent());
						codiceAmministrazioneRicezione = XmlFieldUtil
								.cleanField(((InterBean) busta).getConfermaricezione().getIdentificatore().getCodiceAmministrazione().getContent());
					} else {
						busta.addErroreXML(AbstractInterbean.ERRORE_XML_IDENTIFICATORE);
					}
					break;
				case AGGIORNAMENTO_CONFERMA:
					if (((InterBean) busta).getAggiornamentoconferma() != null && ((InterBean) busta).getAggiornamentoconferma().getMessaggioRicevuto() != null
							&& ((InterBean) busta).getAggiornamentoconferma().getMessaggioRicevuto().getIdentificatore() != null) {
						// si ricavano gli attributi dall'elemento messaggio
						// ricevuto per identificare il protocollo inviato
						numeroRegistrazioneInvioS = XmlFieldUtil.cleanField(((InterBean) busta).getAggiornamentoconferma().getMessaggioRicevuto()
								.getIdentificatore().getNumeroRegistrazione().getContent());
						dataRegistrazioneInvioS = XmlFieldUtil.cleanField(
								((InterBean) busta).getAggiornamentoconferma().getMessaggioRicevuto().getIdentificatore().getDataRegistrazione().getContent());
						siglaRegistroInvio = XmlFieldUtil.cleanField(
								((InterBean) busta).getAggiornamentoconferma().getMessaggioRicevuto().getIdentificatore().getCodiceRegistro().getContent());
						codiceAOOInvio = XmlFieldUtil.cleanField(
								((InterBean) busta).getAggiornamentoconferma().getMessaggioRicevuto().getIdentificatore().getCodiceAOO().getContent());
						codiceAmministrazioneInvio = XmlFieldUtil.cleanField(((InterBean) busta).getAggiornamentoconferma().getMessaggioRicevuto()
								.getIdentificatore().getCodiceAmministrazione().getContent());
					} else {
						busta.addErroreXML(AbstractInterbean.ERRORE_XML_MESSAGGIO_RICEVUTO);
					}
					if (((InterBean) busta).getAggiornamentoconferma() != null && ((InterBean) busta).getAggiornamentoconferma().getIdentificatore() != null) {
						// identificatore del protocollo ricevuto
						numeroRegistrazioneRicezioneS = XmlFieldUtil
								.cleanField(((InterBean) busta).getAggiornamentoconferma().getIdentificatore().getNumeroRegistrazione().getContent());
						dataRegistrazioneRicezioneS = XmlFieldUtil
								.cleanField(((InterBean) busta).getAggiornamentoconferma().getIdentificatore().getDataRegistrazione().getContent());
						siglaRegistroRicezione = XmlFieldUtil
								.cleanField(((InterBean) busta).getAggiornamentoconferma().getIdentificatore().getCodiceRegistro().getContent());
						codiceAOORicezione = XmlFieldUtil
								.cleanField(((InterBean) busta).getAggiornamentoconferma().getIdentificatore().getCodiceAOO().getContent());
						codiceAmministrazioneRicezione = XmlFieldUtil
								.cleanField(((InterBean) busta).getAggiornamentoconferma().getIdentificatore().getCodiceAmministrazione().getContent());
					} else {
						busta.addErroreXML(AbstractInterbean.ERRORE_XML_IDENTIFICATORE);
					}
					break;
				case NOTIFICA_ECCEZIONE:
					if (((InterBean) busta).getNotificaeccezione() != null && ((InterBean) busta).getNotificaeccezione().getMessaggioRicevuto() != null
							&& ((InterBean) busta).getNotificaeccezione().getMessaggioRicevuto().getIdentificatore() != null) {
						// si ricavano gli attributi dall'elemento messaggio
						// ricevuto per identificare il protocollo inviato
						numeroRegistrazioneInvioS = XmlFieldUtil.cleanField(
								((InterBean) busta).getNotificaeccezione().getMessaggioRicevuto().getIdentificatore().getNumeroRegistrazione().getContent());
						dataRegistrazioneInvioS = XmlFieldUtil.cleanField(
								((InterBean) busta).getNotificaeccezione().getMessaggioRicevuto().getIdentificatore().getDataRegistrazione().getContent());
						siglaRegistroInvio = XmlFieldUtil.cleanField(
								((InterBean) busta).getNotificaeccezione().getMessaggioRicevuto().getIdentificatore().getCodiceRegistro().getContent());
						codiceAOOInvio = XmlFieldUtil
								.cleanField(((InterBean) busta).getNotificaeccezione().getMessaggioRicevuto().getIdentificatore().getCodiceAOO().getContent());
						codiceAmministrazioneInvio = XmlFieldUtil.cleanField(
								((InterBean) busta).getNotificaeccezione().getMessaggioRicevuto().getIdentificatore().getCodiceAmministrazione().getContent());
					} else {
						busta.addErroreXML(AbstractInterbean.ERRORE_XML_MESSAGGIO_RICEVUTO);
					}
					...
					// andato in errore
					if (((InterBean) busta).getNotificaeccezione() != null && ((InterBean) busta).getNotificaeccezione().getMotivo() != null) {
						// si registra il motivo di eccezione
						String motivoEccezione = XmlFieldUtil.cleanField(((InterBean) busta).getNotificaeccezione().getMotivo().getContent());
				...
				case ANNULLAMENTO_PROTOCOLLAZIONE:
					...
					// protocollo ricevuto
					if (((InterBean) busta).getAnnullamentoprotocollazione() != null
							&& ((InterBean) busta).getAnnullamentoprotocollazione().getIdentificatore() != null) {
						numeroRegistrazioneRicezioneS = XmlFieldUtil
								.cleanField(((InterBean) busta).getAnnullamentoprotocollazione().getIdentificatore().getNumeroRegistrazione().getContent());
						dataRegistrazioneRicezioneS = XmlFieldUtil
								.cleanField(((InterBean) busta).getAnnullamentoprotocollazione().getIdentificatore().getDataRegistrazione().getContent());
						siglaRegistroRicezione = XmlFieldUtil
								.cleanField(((InterBean) busta).getAnnullamentoprotocollazione().getIdentificatore().getCodiceRegistro().getContent());
						codiceAOORicezione = XmlFieldUtil
								.cleanField(((InterBean) busta).getAnnullamentoprotocollazione().getIdentificatore().getCodiceAOO().getContent());
						codiceAmministrazioneRicezione = XmlFieldUtil
								.cleanField(((InterBean) busta).getAnnullamentoprotocollazione().getIdentificatore().getCodiceAmministrazione().getContent());
					} else {
						busta.addErroreXML(AbstractInterbean.ERRORE_XML_IDENTIFICATORE);
					}
					break;
				case SEGNATURA:
					...
					if (((InterBean) busta).getSegnatura() != null && ((InterBean) busta).getSegnatura().getIntestazione() != null
							&& ((InterBean) busta).getSegnatura().getIntestazione().getIdentificatore() != null) {
						// identificatore del protocollo
						siglaRegistroRicezione = XmlFieldUtil
								.cleanField(((InterBean) busta).getSegnatura().getIntestazione().getIdentificatore().getCodiceRegistro().getContent());
						numeroRegistrazioneRicezioneS = XmlFieldUtil
								.cleanField(((InterBean) busta).getSegnatura().getIntestazione().getIdentificatore().getNumeroRegistrazione().getContent());
						dataRegistrazioneRicezioneS = XmlFieldUtil
								.cleanField(((InterBean) busta).getSegnatura().getIntestazione().getIdentificatore().getDataRegistrazione().getContent());
						codiceAOORicezione = XmlFieldUtil
								.cleanField(((InterBean) busta).getSegnatura().getIntestazione().getIdentificatore().getCodiceAOO().getContent());
						codiceAmministrazioneRicezione = XmlFieldUtil
								.cleanField(((InterBean) busta).getSegnatura().getIntestazione().getIdentificatore().getCodiceAmministrazione().getContent());
						oggettoRegistrazioneRicezione = XmlFieldUtil.cleanField(((InterBean) busta).getSegnatura().getIntestazione().getOggetto().getContent());
					} else {
						busta.addErroreXML(AbstractInterbean.ERRORE_XML_IDENTIFICATORE);
					}
					break;
				...
				if (dataRegistrazioneInvioS != null && !StringUtils.isEmpty(dataRegistrazioneInvioS)) {
					...
					if (((InterBean) busta).getType() != InteroperabilitaType.SEGNATURA
							&& ((InterBean) busta).getType() != InteroperabilitaType.ANNULLAMENTO_PROTOCOLLAZIONE) {
				...
				if (siglaRegistroInvio == null || StringUtils.isEmpty(siglaRegistroInvio)) {
					siglaRegistroInvio = null;
					if (((InterBean) busta).getType() != InteroperabilitaType.SEGNATURA
							&& ((InterBean) busta).getType() != InteroperabilitaType.ANNULLAMENTO_PROTOCOLLAZIONE) {
						// Segnatura e annullamento non hanno il campo e quindi
						// l'errore non va registrato
						busta.addErroreXML(InterBean.ERRORE_XML_CODICE_REGISTRO_INVIO);
					}
				}
				...
				if (dataRegistrazioneRicezioneS != null && !StringUtils.isEmpty(dataRegistrazioneRicezioneS)) {
					...
					if (((InterBean) busta).getType() != InteroperabilitaType.NOTIFICA_ECCEZIONE) {
				...
				if (siglaRegistroRicezione == null || StringUtils.isEmpty(siglaRegistroRicezione)) {
					siglaRegistroRicezione = null;
					if (((InterBean) busta).getType() != InteroperabilitaType.NOTIFICA_ECCEZIONE) {
						// Eccezione non ha il campo e quindi l'errore non va
						// registrato
						busta.addErroreXML(InterBean.ERRORE_XML_CODICE_REGISTRO_RICEZIONE);
					}
				}
				...
				if (((InterBean) busta).getType() == InteroperabilitaType.SEGNATURA) {
					...
					if (siglaRegistroRicezione != null) {
						emailBean.getMail().setSiglaRegistroMitt(siglaRegistroRicezione);
					}
					...
					if (((InterBean) busta).getType() != InteroperabilitaType.ANNULLAMENTO_PROTOCOLLAZIONE) {
						...
						if (annoRegistrazioneInvio != null && numeroRegistrazioneInvio != null) {
							...
							if (listaMail != null && listaMail.size() > 0) {
								...
								for (TEmailMgo mailProtocollo : listaMail) {
									...
									try {
										...
										try {
											...
											// disposizione a parte il codice
											// registro
											if (((InterBean) busta).getType() != InteroperabilitaType.NOTIFICA_ECCEZIONE && idDestinatario != null
												...
												if (listRegExt == null || listRegExt.size() == 0) {
													...
													listaRegistrazioniEsterneToInsert
															.add(creaRegistrazioneEsterna(numeroRegistrazioneRicezione, dataRegistrazioneRicezione,
																	annoRegistrazioneRicezione, codiceAOORicezione, codiceAmministrazioneRicezione,
																	siglaRegistroRicezione, idEmailInProcess, idDestinatario, idEmailProtocollo));
												} else {
													...
													listaRegistrazioniEsterneToUpdate.add(
															updateRegistrazioneEsterna(regEsterna, numeroRegistrazioneRicezione, dataRegistrazioneRicezione,
																	annoRegistrazioneRicezione, codiceAOORicezione, codiceAmministrazioneRicezione,
																	siglaRegistroRicezione, idEmailInProcess, idDestinatario, idEmailProtocollo));
												}
											} else {
												if (((InterBean) busta).getType() != InteroperabilitaType.NOTIFICA_ECCEZIONE) {
													...
													log.warn("Sigla Registro protocollo ricevuto: " + siglaRegistroRicezione);
													log.warn("Tipologia notifica interoperabile: " + ((InterBean) busta).getType());
												} else {
													log.warn(
								...
								log.warn("Sigla Registro protocollo inviato: " + siglaRegistroInvio);
								log.warn("Tipologia notifica interoperabile: " + ((InterBean) busta).getType());
							}
					...
					// si crea una nuova relazione esterna
						...
						if (annoRegistrazioneRicezione != null && numeroRegistrazioneRicezione != null && codiceAOORicezione != null
							...
							// se il codice registro è NULL allora occorre
							// impostare che il filtro SQL sia proprio NULL
							if (siglaRegistroRicezione == null) {
								regEsterna.setSiglaRegistro(DaoTRegEstVsEmail.NULL_CONVENTION);
							} else {
								regEsterna.setSiglaRegistro(siglaRegistroRicezione);
							}
					...
					interopBean.setMailAgganciate(restituisciMailAgganciateAggiornate(emailBean.getRelMail(), ((InterBean) busta).getType()));
					// logiche per il cambio di folder
					interopBean = eseguiLogicheCambioFolder(interopBean, ((InterBean) busta).getType(), emailBean.getMail().getIdCasella(), idMailBox);
	 ...
	 * Si verifica se la mail è effettivamente associata alla mailbox che si sta processando I risultati possono essere molteplici visto che uno stesso
	 * protocollo potrebbe essere stato mandato più volte in differenti mail
	 *
	 * @param idAccount
	...
	private List<TEmailMgo> getListaMailProtocolloInviato(String idMailbox, String idAccount, Integer numeroRegistrazione, Short annoRegistrazione,
			String siglaRegistro) throws Exception {
		...
		try {
			...
			// se la sigla registro non è valorizzata allora si ricerca solo per
			// anno e numero registro
			if (siglaRegistro != null) {
				// Se sigla registro è uguale a Prot. Gen. allora si richiede
				// che la categoria sia PG
				if (siglaRegistro.equalsIgnoreCase(PROTOCOLLO_GENERALE_VALUE)) {
					criteriaMailProtocollo.add(Restrictions.eq("prot.categoriaReg", PROTOCOLLO_GENERALE_ACRONIMO));
				} else {
					// altrimenti ricerca per sigla registro
					criteriaMailProtocollo.add(Restrictions.eq("prot.siglaRegistro", siglaRegistro));
				}
			}
	 ...
	 * destinatari che coincidono con l'account della mail che si sta processando <br>
	 * In mancanza di risultati si ricercano eventuali voci di rubrica associate a codice di amministrazione e codice AOO, visto che la mail di protocollazione
	 * potrebbe essere stata spedita a un indirizzo differente
	 * 
	 * @param idMailBox
	 ...
	 * @param siglaRegistroRegistrazione
	...
	private TRegEstVsEmailBean creaRegistrazioneEsterna(Integer numeroRegistrazioneRicezione, Date dataRegistrazioneRicezione, Short annoRegistrazioneRicezione,
			String AOORicezione, String amministrazione, String siglaRegistroRegistrazione, String idEmailRicevuta, String idDestinatarioMail,
		...
		nuovaReg.setSiglaRegistro(siglaRegistroRegistrazione);
	...
	private TRegEstVsEmailBean updateRegistrazioneEsterna(TRegEstVsEmailBean regEsterna, Integer numeroRegistrazioneRicezione, Date dataRegistrazioneRicezione,
			Short annoRegistrazioneRicezione, String AOORicezione, String amministrazione, String siglaRegistroRegistrazione, String idEmailRicevuta,
			String idDestinatarioMail, String idEmailInviata) throws Exception {
		regEsterna.setAnnoReg(annoRegistrazioneRicezione);
		regEsterna.setNumReg(numeroRegistrazioneRicezione);
		regEsterna.setCodAoo(AOORicezione);
		regEsterna.setCodAmministrazione(amministrazione);
		regEsterna.setSiglaRegistro(siglaRegistroRegistrazione);
		regEsterna.setTsReg(dataRegistrazioneRicezione);

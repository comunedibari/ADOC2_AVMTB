...
public class DeleteMailReceiver extends ImapMailReceiver {
	...
	// forza la cancellazione ma considerando il delay
	private Boolean forceDeleteMessageWithDelay = false;
	...
	public DeleteMailReceiver(String url) {
		super(url);
	...
	public DeleteMailReceiver(String url, String idMailbox) {
		this(url);
		this.idMailbox = idMailbox;
	}
	...
	protected Message[] searchForNewMessages() throws MessagingException {
		...
		if (folder.isOpen()) {
			...
			countMessageFolder = imapFolder.getMessageCount();

			mLogger.debug("Messaggi presenti inizialmente nella folder: " + imapFolder.getMessageCount());
			...
			if (!forceDeleteMessageFinished) {
				...
				for (Integer index = 0; index < listMessageToDeleteInDb.size() && !limitOver; index++) {
					...
					try {
						...
						if (uid != null && uidValidity != null && uidValiditySaved != null && uidValidity.equals(uidValiditySaved)) {
							...
							// cerco prima per uid
							messageUID = imapFolder.getMessageByUID(uid);
						}

						if (messageUID == null) {
							// ricerco per messageid
							mLogger.info("Ricerco per MessageID");
							msgToDelete = imapFolder.search(new MessageIdSearchTerm(messageId));
							uid = null;
							uidValidity = null;
						} else {
						...
						if (msgToDelete != null) {
							...
							if (msgToDelete.length == 0) {
								...
								Boolean deleted = this.deleteMsg(msgToDelete[0], messageId, uid, uidValidity, imapFolder);
				...
				mLogger.debug("Messaggi presenti nella folder dopo la prima fase di cancellazione: " + imapFolder.getMessageCount());

			} else {
				mLogger.debug("Messaggi presenti nella folder: " + imapFolder.getMessageCount());
			}

			if (!limitOver && imapFolder.getMessageCount() > 0) {
...
//				mLogger.info("UIDValidity salvata in database: " + uidValiditySaved);
//
//				Long uidValidity = imapFolder.getUIDValidity();
//
//				mLogger.info("UIDValidity corrente: " + uidValidity);
//
//				Long endUID = IMAPFolder.LASTUID; // vado fino all'ultimo messaggio presente, -1 se non valorizzato
//
//				// se la validità degli UID della folder non è variata allora posso considerare l'ultimo UID salvato per limitare la ricerca dei nuovi messaggi,
//				// quelli successivi si presume che siano tutti nuovi messaggi
//				if (searchByUID && uidValidity != null && uidValiditySaved != null && uidSaved != null && uidValidity.equals(uidValiditySaved)) {
//					mLogger.info("E' attiva la ricerca per UID");
//					endUID = uidSaved;
//				}
//				
//				Message[] messagesFolder = imapFolder.getMessagesByUID(1, endUID);
//
//				mLogger.info("Messaggi ricavati nell'intervallo 1 - " + endUID + ": " + messagesFolder.length);
				// DiegoL: recupero tutti i messaggi senza la verifica degli uid che potrebbe portare alla mancata cancellazione di alcune mail
				// Jira: https://production.eng.it/jira/browse/AURIGA-259
				Message[] messagesFolder = imapFolder.getMessages();
						...
						if (StringUtils.isNotEmpty(messageId)) {
							...
							try {
								...
								if ((mailBoxMessage = mailBusiness.getMailBoxMessageWithOperations(messageId, this.idMailbox)) != null) {
									...
									if ((listIdMessageToDeleteInDb.contains(messageId) || mailBusiness.canBeDeletedMailboxMessage(mailBoxMessage) && canBeDeleteMsg(message.getSentDate(), hourDelay)
											|| (forceDeleteMessageFinished && isPresentInTemailMgo(consoleType, messageId,
													configBean.getAccount().getAccountAddress(), idMailbox, idAccount)) || 
											(forceDeleteMessageWithDelay && isPresentInTemailMgo(consoleType, messageId,
													configBean.getAccount().getAccountAddress(), idMailbox, idAccount) && canBeDeleteMsg(message.getSentDate(), hourDelay)))) {

										Boolean deleted = this.deleteMsg(message, messageId, mailBoxMessage.getMessageUid(),
												mailBoxMessage.getMailboxUidValidity(), imapFolder);
	...
	private Boolean isPresentInTemailMgo(String consoleType, String messageId, String accountAddress, String idMailBox, String idAccount)
		...
		try {
			...
			String lString = null;
			boolean isToBeDeletedHours = false;
			...
			if (consoleType != null && consoleType.equalsIgnoreCase("egr")) {
				...
				// DiegoL: eliminato il controllo sul delay
				// https://production.eng.it/jira/browse/AURIGA-259
	...
	private boolean canBeDeleteMsg(Date  dataInvioMsg, int hours) {
		boolean result = false;                                   
		Calendar cal = GregorianCalendar.getInstance();
		cal.setTime(new Date());
		cal.add(Calendar.HOUR_OF_DAY, -hours);
		result = cal.getTime().after(dataInvioMsg);
		logger.debug("Data Riferimento: " + cal.getTime());
		logger.debug("Data messaggio: " + dataInvioMsg);
		logger.debug("canBeDeleteMsg.result: " + result);
		return result;
	}
	...
	public Boolean getForceDeleteMessageWithDelay() {
		return forceDeleteMessageWithDelay;
	}

	public void setForceDeleteMessageWithDelay(Boolean forceDeleteMessageWithDelay) {
		this.forceDeleteMessageWithDelay = forceDeleteMessageWithDelay;
	}

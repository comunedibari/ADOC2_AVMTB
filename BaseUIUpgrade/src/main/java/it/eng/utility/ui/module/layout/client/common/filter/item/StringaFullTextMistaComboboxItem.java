...
public class StringaFullTextMistaComboboxItem extends CustomItem {
	...
	private ComboBoxItem createComboBox() {
		...
		paroleEditorType.setItemHoverFormatter(new FormItemHoverFormatter() {
		...
		paroleEditorType.setFetchDelay(500);
		...
		lPickListProperties.addCellClickHandler(new CellClickHandler() {
			...
			public void onCellClick(CellClickEvent event) {
				...
				if (getCustomProperties().get("displayField") != null) {
					parole.updateInternalValue("parole", event.getRecord().getAttribute(getCustomProperties().get("displayField")).replace(" ", ";"));
				}

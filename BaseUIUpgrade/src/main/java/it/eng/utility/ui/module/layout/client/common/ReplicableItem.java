...
public abstract class ReplicableItem extends CanvasItem implements HasChangeCanEditHandlers {
	...
	public synchronized void manageChanged(final ChangedEvent event, final ReplicableCanvas lReplicableCanvas) {
		...
		// final int[] selectionRange = (event.getItem() instanceof TextItem) ? ((TextItem) event.getItem()).getSelectionRange() : null;
		updateInternalValue(event.getItem().getName(), event.getValue(), lReplicableCanvas.getHLayout());
		// if (event.getItem() instanceof TextItem) {
		// if (selectionRange != null) {
		// Scheduler.get().scheduleDeferred(new ScheduledCommand() {
		//
		// @Override
		// public void execute() {
		// ((TextItem) event.getItem()).setSelectionRange(selectionRange[0], selectionRange[1]);
		// }
		// });
		// }
		// }

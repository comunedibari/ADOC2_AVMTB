...
//		GWT.create(BeanFactory.FormItemMetaFactory.class);
		new OneCallGWTRestService<Record, Record>("ConfigurationDataSource").call(new Record(), new ServiceCallback<Record>() {
			...
			public void execute(Record record) {
				...
				/*******************************************************************************************/
				Map<String, ArrayList<Map<String, String>>> lAppletParameters = record.getAttributeAsRecord("appletConfig").getAttributeAsMap(
						"appletConfiguration");
...
//			}
	...
	public static void addPortlet(final String nomeEntita, final String title, final String icon, final Canvas body) {
		...
		if (openedPortlet == null) {
			if (showMaximizedPortlet(nomeEntita)) {
				...
				buildAndShowPortlet(nomeEntita, title, icon, body, true, "0", "0", String.valueOf(config.getDefaultPortletWidth()),
						String.valueOf(config.getDefaultPortletHeight()), false);
					...
					public void execute(DSResponse response, Object rawData, DSRequest request) {
						...
						if (data.length > 0 && data[0].getAttribute("value") != null) {
							...
							buildAndShowPortlet(nomeEntita, title, icon, body, record.getAttributeAsBoolean("maximized"), record.getAttribute("pageLeft"),
									record.getAttribute("pageTop"), record.getAttribute("width"), record.getAttribute("height"), false);
						} else {
								...
								public void execute(DSResponse response, Object rawData, DSRequest request) {
									...
									if (data.length > 0 && data[0].getAttribute("value") != null) {
										...
										buildAndShowPortlet(nomeEntita, title, icon, body, record.getAttributeAsBoolean("maximized"),
												...
												record.getAttribute("height"), false);
									} else {
										int left = 5;
										int top = 5;
										...
										buildAndShowPortlet(nomeEntita, title, icon, body, false, String.valueOf(offset ? left + 20 : left),
												...
												String.valueOf(config.getDefaultPortletHeight()), offset);
									}
								}
							});
						}
	...
	public static void buildAndShowPortlet(final String nomeEntita, String title, String icon, Canvas body, boolean maximized, String pageLeft, String pageTop,
			String width, String height, boolean offset) {
		...
		try {
			...
			if (body != null) {
				...
				addPortletToPortalDesktop(portlet);
...
//				}
	...
	public static void addPortletToPortalDesktop(Portlet portlet) {
...
//		}, 750);
	...
	public static void showConfirmDialogWithWarning(String title, String warningMessage, final String confermaButtonTitle, String annullaButtonTitle, final BooleanCallback callback) {
		if (UserInterfaceFactory.isAttivaAccessibilita()) {
			...
			SC.ask("Attenzione", warningMessage, callback, dialogProperties);
		} else {
			final Dialog dialog = new Dialog();
			Button confermaButton = new Button(confermaButtonTitle != null && !"".equals(confermaButtonTitle) ? confermaButtonTitle : "Ok");
			...
			dialog.addButtonClickHandler(new ButtonClickHandler() {
				...
				public void onButtonClick(ButtonClickEvent event) {
					...
					if(callback != null) {
						callback.execute(buttonTitle != null && buttonTitle.equalsIgnoreCase(confermaButtonTitle));
					}

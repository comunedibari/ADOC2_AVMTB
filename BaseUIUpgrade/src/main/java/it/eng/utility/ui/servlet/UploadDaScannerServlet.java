...
public class UploadDaScannerServlet {
	...
	public ResponseEntity<String> post(@RequestParam("idSelected") String idSelected,
			...
			@RequestParam("userfiletif") MultipartFile userfiletif,
		...
		try {
			...
			String fileSavedTif = StringEscapeUtils.escapeJava(filename+"#"+lStrPath);
			...
			lStringBuffer.append(fileSavedPDF + "---" + fileSavedTif + "---" + result);
	...
	private MimeTypeFirmaBean retrieveInfo(InputStream pInputStream, String pStrDisplayFileName, String lStrPath) throws StorageException{
		...
		lMimeTypeFirmaBean.setMimetype("application/pdf");
		lMimeTypeFirmaBean.setDaScansione(true);
		lMimeTypeFirmaBean.setBytes(storageService.getRealFile(lStrPath).length());
